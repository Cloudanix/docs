...
2. In the S3 dashboard, select the bucket you want to check.
3. Click on the 'Permissions' tab.
4. Under the 'Access Control List (ACL)' section, check the 'Everyone' and 'Authenticated Users' groups. If 'Write ACP' is enabled for these groups, then the S3 bucket allows WRITE_ACP access to authenticated users.
...
3. Check bucket policy: For each bucket, you need to check the bucket policy to see if it allows WRITE_ACP access to authenticated users. You can do this by running the following command for each bucket.

   ```
   aws s3api get-bucket-policy --bucket bucket-name --query 'Policy'
   ```

4. Analyze the policy: The above command will return the bucket policy in a JSON format. You need to analyze this policy to see if it contains a statement that allows "s3:PutBucketAcl" or "s3:PutObjectAcl" actions for authenticated users (Principal: "*"). If such a statement exists, then the bucket allows WRITE_ACP access to authenticated users.
