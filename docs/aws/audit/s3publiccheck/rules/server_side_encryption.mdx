...
4. In the Default encryption section, check if Amazon S3 Server Side Encryption (SSE) is enabled. If it is not, then the bucket does not enforce server-side encryption.
...
3. Check the bucket policy: For each bucket, you need to check if server-side encryption is enforced. You can do this by running the following command for each bucket.

   ```
   aws s3api get-bucket-policy --bucket <bucket-name>
   ```

4. Check for server-side encryption: In the output of the above command, look for the `Statement` section. If server-side encryption is enforced, you should see a statement similar to the following:
   ...
   If you don't see this statement, it means that server-side encryption is not enforced for the bucket.
...
2. Create a Python script to list all S3 buckets and check their encryption configuration:
   You can use the following script to list all S3 buckets and check if they have server-side encryption enabled.
   ```python
   import boto3
       ...
       try:
           ...
           for rule in rules:
               if 'ApplyServerSideEncryptionByDefault' in rule:
                   print(f"Bucket {bucket['Name']} has server-side encryption enabled.")
       except Exception as e:
           print(f"Bucket {bucket['Name']} does not have server-side encryption enabled.")
   ```
   
3. Run the Python script:
   You can run the Python script using the following command in your terminal:
   ```
   python check_s3_encryption.py
   ```
   This will print out the names of all S3 buckets and whether they have server-side encryption enabled or not.

4. Analyze the output:
   If any of your S3 buckets do not have server-side encryption enabled, you should consider enabling it to protect your data at rest. If all your S3 buckets have server-side encryption enabled, then you are following best practices for data protection.
