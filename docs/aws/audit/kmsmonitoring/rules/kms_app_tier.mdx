...
3. In the KMS dashboard, select "Customer managed keys" from the left-hand side menu. This will display a list of all the KMS keys that are currently in use.
4. Check the "Key Usage" column for each key. If the App-tier KMS key is in use, it should be listed here. If it's not listed, then it's not in use.
...
2. Once AWS CLI is installed and configured, you can list all the KMS keys in your account by running the following command:
...
3. To check if a specific KMS key is in use, you can use the 'list-grants' command. Replace 'key-id' with the ID of the key you want to check:
   ...
   ```
   This command will return a list of all grants for the specified key. If the list is empty, it means the key is not in use.

4. To automate this process, you can write a Python script using the boto3 library, which is the Amazon Web Services (AWS) Software Development Kit (SDK) for Python. This script would iterate over all the keys in your account and check if they are in use. Here is a basic example:
   ...
   ```
   This script will print the IDs of all keys that are not in use.
...
3. Write a Python script to list all KMS keys and check their usage:

   ```python
   import boto3

   def check_kms_key_usage():
   ...
   This script lists all KMS keys and checks if they are enabled. If a key is not enabled, it is considered not in use and its ID is printed.

4. Run the script: You can run the script using the Python interpreter:

   ```
   python check_kms_key_usage.py
   ```

   This will print the IDs of all KMS keys that are not in use. If no such keys are found, it will not print anything.
