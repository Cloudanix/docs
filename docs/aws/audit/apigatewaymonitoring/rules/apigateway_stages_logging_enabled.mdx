...
2. In the navigation pane, choose the API Gateway you want to inspect.

3. In the main pane, select the "Stages" option from the left-hand side menu.

4. Select the stage you want to inspect. In the "Logs/Tracing" tab, check if the "Enable CloudWatch Logs" option is enabled. If it's not enabled, it means the API Gateway Execution Logging is not enabled for that particular stage.

#### Using CLI

1. First, you need to list all the APIs in your AWS account. You can do this by using the following AWS CLI command:

```bash
aws apigateway get-rest-apis --region your-region
```
This command will return a list of all the APIs in your AWS account in the specified region.

2. Once you have the list of APIs, you can check the logging settings for each API. You can do this by using the following AWS CLI command:

```bash
aws apigateway get-stage --rest-api-id your-rest-api-id --stage-name your-stage-name --region your-region
```
This command will return the settings for the specified stage of the specified API. You need to replace 'your-rest-api-id' and 'your-stage-name' with the actual ID and name of your API and stage.

3. In the output of the above command, look for the 'methodSettings' section. Under this section, check the value of the 'loggingLevel' field. If the value is 'OFF', then execution logging is not enabled for the API.

4. Repeat steps 2 and 3 for each API and stage in your AWS account to check if execution logging is enabled for all APIs.
...
2. Import the necessary libraries and initialize the API Gateway client: You need to import Boto3 and initialize the API Gateway client to interact with the AWS API Gateway service.
...
3. Fetch the list of APIs: Use the `get_rest_apis` method to fetch the list of all APIs in your AWS account.
...
4. Check if Execution Logging is enabled: For each API, check if the `loggingLevel` is set to `INFO` or `ERROR`. If it's not, then execution logging is not enabled for that API.

   ```python
   for api in apis['items']:
       # Get the API Gateway stage details
       stages = client.get_stages(restApiId=api['id'])
       for stage in stages['item']:
           # Check if logging is enabled
           if 'methodSettings' in stage:
               for settings in stage['methodSettings'].values():
                   if 'loggingLevel' not in settings or settings['loggingLevel'] not in ['INFO', 'ERROR']:
                       print(f"Execution logging is not enabled for API: {api['name']}, Stage: {stage['stageName']}")
   ```
   
This script will print out the names of all APIs and their stages where execution logging is not enabled.
