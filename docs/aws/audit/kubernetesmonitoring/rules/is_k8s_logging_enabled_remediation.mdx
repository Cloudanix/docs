...
4. On the Clusters page, select the name of the cluster that you want to check.
5. In the Logging section, check if the CloudWatch Logs are enabled. If the status is Disabled, then the logging feature is not enabled for the selected Amazon EKS cluster.
...
3. To check the logging configuration of a specific EKS cluster, use the following command:
   ```
   aws eks describe-cluster --name <cluster-name> --query "cluster.logging.clusterLogging"
   ```
   Replace `<cluster-name>` with the name of your EKS cluster. This command will return the logging configuration of the specified EKS cluster.

4. The output of the above command will show whether the logging is enabled or not. If the "enabled" field is set to "true", then logging is enabled. If it's set to "false", then logging is not enabled. 

Remember to replace `<cluster-name>` with the actual name of your EKS cluster.
...
3. Create a function to check EKS logging: This function will use the boto3 library to interact with the EKS service and check if logging is enabled for each EKS cluster.
   ...
   def check_eks_logging():
       ...
       for cluster in clusters:
           ...
           logging_status = response['cluster']['logging']['clusterLogging']
           for types in logging_status:
               if types['enabled']:
                   print(f"Logging is enabled for {types['types']} in cluster {cluster}")
               else:
                   print(f"Logging is not enabled for {types['types']} in cluster {cluster}")
   ```

4. Call the function: Finally, you can call the function to check the logging status of all EKS clusters.

   ```python
   check_eks_logging()
   ```
   
This script will print out the logging status for each type of logs (like "api", "audit", "authenticator", etc.) for each EKS cluster. If logging is not enabled for a certain type, it will print that out, helping you detect any misconfigurations.
