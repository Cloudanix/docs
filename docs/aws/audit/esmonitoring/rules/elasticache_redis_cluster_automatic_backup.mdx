---
slug: elasticache_redis_cluster_automatic_backup
title: Elasticache Clusters Should Have Automatic Backup 
sidebar_label: Elasticache Clusters Should Have Automatic Backup 
---

### More Info:

This rule checks if the Amazon ElastiCache Redis clusters have automatic backup turned on. The rule is NON_COMPLIANT if the SnapshotRetentionLimit for Redis cluster is less than the SnapshotRetentionPeriod parameter. For example: If the parameter is 15 then the rule is non-compliant if the snapshotRetentionPeriod is between 0-15.

### Risk Level

High

### Address

Configuration

### Compliance Standards

CBP,SEBI,RBI_MD_ITF,RBI_UCB


### Triage and Remediation
<Tabs>



<Tab title='Prevention'>
### How to Prevent
<AccordionGroup>
<Accordion title='Using Console' defaultOpen='true'>
To prevent the misconfiguration where ElastiCache clusters should have automatic backups enabled in AWS, follow these steps using the AWS Management Console:

1. **Navigate to the ElastiCache Dashboard:**
   - Sign in to the AWS Management Console.
   - In the Services menu, select "ElastiCache" under the "Database" category.

2. **Select the Cluster:**
   - In the ElastiCache dashboard, click on "Clusters" in the left-hand navigation pane.
   - Select the cluster for which you want to enable automatic backups.

3. **Modify the Cluster:**
   - With the cluster selected, click on the "Modify" button at the top of the page.
   - In the "Modify Cluster" page, scroll down to the "Backup" section.

4. **Enable Automatic Backups:**
   - Check the box for "Enable automatic backups."
   - Set the "Backup Retention Period" to the desired number of days (e.g., 1-35 days).
   - Click the "Modify" button at the bottom of the page to save your changes.

By following these steps, you ensure that automatic backups are enabled for your ElastiCache clusters, helping to prevent data loss and misconfigurations.
</Accordion>

<Accordion title='Using CLI'>
To prevent the misconfiguration where ElastiCache clusters should have automatic backups enabled using AWS CLI, follow these steps:

1. **Check Current Backup Settings:**
   First, you need to check the current backup settings for your ElastiCache cluster. Use the following command to describe your ElastiCache cluster and see if automatic backups are enabled.

   ```sh
   aws elasticache describe-cache-clusters --cache-cluster-id <your-cluster-id> --show-cache-node-info
   ```

2. **Modify the Cluster to Enable Automatic Backups:**
   If automatic backups are not enabled, you can modify the cluster to enable them. Use the following command to set the `SnapshotRetentionLimit` parameter, which specifies the number of days for which ElastiCache retains automatic snapshots before deleting them.

   ```sh
   aws elasticache modify-cache-cluster --cache-cluster-id <your-cluster-id> --snapshot-retention-limit <number-of-days> --apply-immediately
   ```

3. **Verify the Changes:**
   After modifying the cluster, verify that the changes have been applied correctly by describing the cluster again.

   ```sh
   aws elasticache describe-cache-clusters --cache-cluster-id <your-cluster-id> --show-cache-node-info
   ```

4. **Automate the Configuration for Future Clusters:**
   To ensure that all future ElastiCache clusters have automatic backups enabled by default, you can create a script or use AWS CloudFormation templates that include the `SnapshotRetentionLimit` parameter. Here is an example of a CLI command to create a new cluster with automatic backups enabled:

   ```sh
   aws elasticache create-cache-cluster --cache-cluster-id <new-cluster-id> --engine <engine-type> --cache-node-type <node-type> --num-cache-nodes <number-of-nodes> --snapshot-retention-limit <number-of-days>
   ```

By following these steps, you can ensure that your ElastiCache clusters have automatic backups enabled, thereby preventing the misconfiguration.
</Accordion>

<Accordion title='Using Python'>
To prevent the misconfiguration of Elasticache Clusters not having automatic backups enabled in AWS using Python scripts, you can use the Boto3 library, which is the AWS SDK for Python. Here are the steps:

1. **Install Boto3 Library**:
   Ensure you have the Boto3 library installed. You can install it using pip if you haven't already.

   ```bash
   pip install boto3
   ```

2. **Set Up AWS Credentials**:
   Make sure your AWS credentials are configured. You can set them up using the AWS CLI or by directly configuring the `~/.aws/credentials` file.

3. **Create a Python Script to Enable Automatic Backups**:
   Write a Python script that will check your Elasticache clusters and enable automatic backups if they are not already enabled.

   ```python
   import boto3

   def enable_automatic_backups(cluster_id, retention_period=7):
       client = boto3.client('elasticache')
       
       # Describe the cluster to check current backup settings
       response = client.describe_cache_clusters(CacheClusterId=cluster_id, ShowCacheNodeInfo=True)
       cluster = response['CacheClusters'][0]
       
       # Check if automatic backups are enabled
       if not cluster['SnapshotRetentionLimit']:
           print(f"Enabling automatic backups for cluster: {cluster_id}")
           client.modify_cache_cluster(
               CacheClusterId=cluster_id,
               SnapshotRetentionLimit=retention_period
           )
           print(f"Automatic backups enabled with a retention period of {retention_period} days.")
       else:
           print(f"Automatic backups are already enabled for cluster: {cluster_id}")

   def main():
       client = boto3.client('elasticache')
       
       # List all clusters
       response = client.describe_cache_clusters(ShowCacheNodeInfo=True)
       clusters = response['CacheClusters']
       
       for cluster in clusters:
           cluster_id = cluster['CacheClusterId']
           enable_automatic_backups(cluster_id)

   if __name__ == "__main__":
       main()
   ```

4. **Run the Script**:
   Execute the script to ensure that all your Elasticache clusters have automatic backups enabled.

   ```bash
   python enable_automatic_backups.py
   ```

This script will iterate through all your Elasticache clusters and enable automatic backups for those that do not have it enabled. The retention period for the backups is set to 7 days by default, but you can adjust it as needed.
</Accordion>

</AccordionGroup>
</Tab>
<Tab title='Cause'>
### Check Cause
<AccordionGroup>
<Accordion title='Using Console' defaultOpen='true'>
1. Sign in to the AWS Management Console.
2. Navigate to the ElastiCache dashboard. You can do this by typing "ElastiCache" into the search bar at the top of the console, then selecting "ElastiCache" from the dropdown menu.
3. In the ElastiCache dashboard, select "Redis" from the left-hand menu. This will bring up a list of all your Redis clusters.
4. For each cluster, check the "Backup" section. If the "Automatic Backup" field is set to "Disabled", then the cluster does not have automatic backup enabled.
</Accordion>

<Accordion title='Using CLI'>
1. Install and configure AWS CLI: Before you can start using AWS CLI, you need to install it on your local system. You can download it from the official AWS website. After installation, you need to configure it with your AWS account credentials. You can do this by running the command `aws configure` and then entering your AWS Access Key ID, Secret Access Key, Default region name, and Default output format when prompted.

2. List all Elasticache Clusters: Use the following AWS CLI command to list all the Elasticache clusters in your AWS account.

   ```
   aws elasticache describe-cache-clusters --query 'CacheClusters[*].[CacheClusterId]'
   ```

   This command will return a list of all the Elasticache cluster IDs.

3. Check Automatic Backup for each Elasticache Cluster: For each Elasticache cluster ID returned by the previous command, use the following AWS CLI command to check if automatic backup is enabled.

   ```
   aws elasticache describe-cache-clusters --cache-cluster-id <CacheClusterId> --query 'CacheClusters[*].[SnapshotRetentionLimit]'
   ```

   Replace `<CacheClusterId>` with the ID of the Elasticache cluster you want to check. This command will return the number of days for which ElastiCache will retain automatic snapshots. If the returned value is 0, it means that automatic backup is not enabled for this cluster.

4. Analyze the output: If the `SnapshotRetentionLimit` for any of the Elasticache clusters is 0, it means that automatic backup is not enabled for that cluster. This is a misconfiguration and should be fixed.
</Accordion>

<Accordion title='Using Python'>
1. Import the necessary AWS SDK for Python (Boto3) modules and establish a session with AWS using your credentials.

```python
import boto3
from botocore.exceptions import BotoCoreError, ClientError

session = boto3.Session(
    aws_access_key_id='YOUR_ACCESS_KEY',
    aws_secret_access_key='YOUR_SECRET_KEY',
    region_name='us-west-2'
)
```

2. Create a client for 'elasticache' service and retrieve all the Elasticache clusters.

```python
elasticache = session.client('elasticache')

try:
    response = elasticache.describe_cache_clusters()
except ClientError as e:
    print(f"Unexpected error: {e}")
```

3. Iterate over each Elasticache cluster and check the 'SnapshotRetentionLimit' attribute. If the value is 0, it means automatic backup is not enabled for that cluster.

```python
for cluster in response['CacheClusters']:
    if cluster['SnapshotRetentionLimit'] == 0:
        print(f"Automatic backup is not enabled for the cluster: {cluster['CacheClusterId']}")
```

4. If you want to check this for all regions, you can get all regions using `ec2.describe_regions()` and repeat the above steps for each region.

Remember to replace `'YOUR_ACCESS_KEY'` and `'YOUR_SECRET_KEY'` with your actual AWS access key and secret key.
</Accordion>

</AccordionGroup>
</Tab>

<Tab title='Remediation'>
### Remediation

<AccordionGroup>
<Accordion title='Using Console' defaultOpen='true'>
To remediate the misconfiguration for AWS ElastiCache clusters not having automatic backups enabled, follow these steps using the AWS Management Console:

1. **Sign in to the AWS Management Console**: Go to https://aws.amazon.com/ and sign in to your AWS account.

2. **Navigate to ElastiCache Dashboard**: Click on the "Services" dropdown menu at the top, and then select "ElastiCache" under the "Database" category.

3. **Select the ElastiCache Cluster**: In the ElastiCache dashboard, select the ElastiCache cluster for which you want to enable automatic backups by clicking on the cluster ID.

4. **Modify Cluster Settings**: In the cluster details page, click on the "Modify" button at the top right corner.

5. **Enable Automatic Backups**: Scroll down to the "Backup" section in the Modify Cluster page. Check the box next to "Automatic backups" to enable automatic backups for the ElastiCache cluster.

6. **Configure Backup Retention Period**: Set the desired backup retention period. You can choose the number of days for which you want to retain the automatic backups.

7. **Save Changes**: Scroll down to the bottom of the page and click on the "Modify" button to save the changes.

8. **Monitor Backup Status**: Once the modifications are saved, monitor the status of the automatic backups in the ElastiCache dashboard to ensure that they are running as expected.

By following these steps, you have successfully enabled automatic backups for your AWS ElastiCache cluster, thereby remediating the misconfiguration.

#
</Accordion>

<Accordion title='Using CLI'>
To remediate the misconfiguration of Elasticache clusters not having automatic backups enabled for AWS Elasticache using the AWS CLI, follow these steps:

1. **Enable automatic backups for the Elasticache cluster**:
   
   Run the following AWS CLI command to modify the Elasticache cluster's backup retention period and enable automatic backups:
   ```
   aws elasticache modify-cache-cluster --cache-cluster-id your-cluster-id --automatic-failover-enabled true --snapshot-retention-limit 7
   ```
   Replace `your-cluster-id` with the actual ID of the Elasticache cluster.

2. **Verify the configuration**:
   
   You can verify that automatic backups are enabled for the Elasticache cluster by describing the cluster using the following command:
   ```
   aws elasticache describe-cache-clusters --cache-cluster-id your-cluster-id
   ```
   Ensure that the `AutomaticFailoverEnabled` parameter is set to `true` and the `SnapshotRetentionLimit` is set to the desired value (in days).

3. **Monitor the backups**:
   
   After enabling automatic backups, monitor the backups to ensure they are being created as expected. You can do this by checking the AWS Management Console or by using the AWS CLI to list the backups:
   ```
   aws elasticache describe-snapshots --cache-cluster-id your-cluster-id
   ```
   This command will list all the snapshots associated with the Elasticache cluster.

By following these steps, you can remediate the misconfiguration of Elasticache clusters not having automatic backups enabled for AWS Elasticache using the AWS CLI.
</Accordion>

<Accordion title='Using Python'>
To enable automatic backups for an Amazon Elasticache cluster using Python, you can use the AWS SDK for Python (Boto3) to interact with the AWS API. Here are the step-by-step instructions to remediate this misconfiguration:

1. Install Boto3:
   Make sure you have Boto3 installed. You can install it using pip:
   ```
   pip install boto3
   ```

2. Configure AWS Credentials:
   Ensure that you have your AWS credentials set up with the necessary permissions to modify Elasticache clusters. You can set up your credentials using the AWS CLI or by setting environment variables.

3. Write Python script:
   Create a Python script with the following code to enable automatic backups for an Elasticache cluster:

   ```python
   import boto3

   # Initialize the Elasticache client
   elasticache_client = boto3.client('elasticache')

   # Specify the name of the Elasticache cluster to enable automatic backups
   cluster_name = 'your-cluster-name'

   # Enable automatic backups for the specified Elasticache cluster
   response = elasticache_client.modify_cache_cluster(
       CacheClusterId=cluster_name,
       AutomaticBackupRetentionDays=1,  # Set the retention period for automatic backups
       PreferredBackupWindow='07:00-09:00',  # Set the preferred backup window
       ApplyImmediately=True
   )

   print(f"Automatic backups enabled for Elasticache cluster: {cluster_name}")
   ```

4. Run the Python script:
   Save the Python script and run it. This script will enable automatic backups for the specified Elasticache cluster.

5. Verify:
   After running the script, you can verify in the AWS Management Console that automatic backups have been enabled for the Elasticache cluster.

By following these steps, you can remediate the misconfiguration of not having automatic backups enabled for an AWS Elasticache cluster using Python and Boto3.
</Accordion>

</AccordionGroup>
</Tab>
</Tabs>

