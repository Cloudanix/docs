...
1. First, you need to install and configure AWS CLI on your local machine. You can do this by following the instructions provided by AWS. Make sure you have the necessary permissions to access the ElasticSearch domains.

2. Once the AWS CLI is installed and configured, you can list all the ElasticSearch domains in your account by running the following command:

   ```
   aws es list-domain-names
   ```
   This command will return a list of all the ElasticSearch domains in your account.

3. For each domain, you can describe the domain configuration by running the following command:

   ```
   aws es describe-elasticsearch-domain --domain-name <your-domain-name>
   ```
   Replace `<your-domain-name>` with the name of your ElasticSearch domain. This command will return the configuration details of the specified domain.

4. In the output of the above command, look for the `VPCOptions` field. If the `VPCOptions` field is not present or if the `VPCOptions.Endpoint` field is not set to a private IP address, then the ElasticSearch domain is not launched with a private VPC endpoint.
...
def check_es_vpc_endpoints():
    ...
    for domain in domains['DomainNames']:
        ...
        if 'VPCOptions' in domain_config['DomainStatus']:
            if 'VPCId' in domain_config['DomainStatus']['VPCOptions']:
                print(f"{domain_name} is launched with a private VPC endpoint.")
            else:
                print(f"{domain_name} is not launched with a private VPC endpoint.")
        else:
            print(f"{domain_name} is not launched with a private VPC endpoint.")
...
4. Run the script: Finally, you can run the script using a Python interpreter. The script will print out the names of the ElasticSearch domains and whether they are launched with a private VPC endpoint.

Please note that this script only checks if a VPC is associated with the Elasticsearch domain, not if it's a private VPC. To check if the VPC is private, you would need to describe the VPC using the EC2 client and check if it has any internet gateways associated with it.
