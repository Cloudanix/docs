...
4. For each cluster, check the "Last Modified" date and time under the "Details" tab. If a cluster has not been modified or accessed for a long period of time, it may be idle and should be considered for termination.

#### Using CLI

1. **Install and Configure AWS CLI**: Before you can start, make sure you have AWS CLI installed and configured on your system. You can install it using pip (Python package manager) with the command `pip install awscli`. After installation, you can configure it using `aws configure` command and providing your AWS credentials.

2. **List all Redshift Clusters**: Use the `describe-clusters` command to list all Redshift clusters in your AWS account. The command is as follows:

    ```
    aws redshift describe-clusters --query 'Clusters[*].[ClusterIdentifier]' --output text
    ```

    This command will list the identifiers of all Redshift clusters.

3. **Check Cluster Status**: For each cluster, check its status using the `describe-clusters` command again, but this time with the `--cluster-identifier` option. The command is as follows:

    ```
    aws redshift describe-clusters --cluster-identifier <your-cluster-identifier> --query 'Clusters[*].[ClusterStatus]' --output text
    ```

    Replace `<your-cluster-identifier>` with the identifier of the cluster you want to check. This command will return the status of the specified cluster.

4. **Check Last Activity Time**: For each cluster, check the last activity time using the `describe-clusters` command again, but this time with the `--cluster-identifier` option and querying for the `LastRestorableTime`. The command is as follows:

    ```
    aws redshift describe-clusters --cluster-identifier <your-cluster-identifier> --query 'Clusters[*].[LastRestorableTime]' --output text
    ```

    Replace `<your-cluster-identifier>` with the identifier of the cluster you want to check. This command will return the last activity time of the specified cluster. If the last activity time is too long ago, the cluster might be idle.
...
4. Iterate over the clusters and check the 'NumberOfNodes' and 'ClusterStatus' attributes. If the 'NumberOfNodes' is greater than 1 and the 'ClusterStatus' is 'available', then the cluster is idle and should be terminated.

```python
for cluster in response['Clusters']:
    if cluster['NumberOfNodes'] > 1 and cluster['ClusterStatus'] == 'available':
        print(f"Idle Redshift Cluster: {cluster['ClusterIdentifier']}")
```

This script will print out the identifiers of all idle Redshift clusters. You can then decide whether to terminate these clusters or not.
