...
2. Once AWS CLI is set up, you can list all the Redshift clusters in your account by running the following command:
...
3. For each cluster identifier, you can check if it is encrypted by running the following command:

   ```
   aws redshift describe-clusters --cluster-identifier <cluster-identifier> --query 'Clusters[*].[Encrypted]' --output text
   ```

   Replace `<cluster-identifier>` with the actual cluster identifier. This command will return `true` if the cluster is encrypted and `false` if it is not.

4. To automate the process, you can write a script that loops through all the cluster identifiers and checks if they are encrypted. Here is a simple Python script that does this:

   ```python
   import subprocess

   # Get list of all cluster identifiers
   ...
   # Check if each cluster is encrypted
   for cluster in clusters:
       encrypted = subprocess.check_output(["aws", "redshift", "describe-clusters", "--cluster-identifier", cluster, "--query", "Clusters[*].[Encrypted]", "--output", "text"]).decode().strip()
       print(f"Cluster {cluster} is encrypted: {encrypted}")
   ```

   This script will print out whether each Redshift cluster is encrypted.
...
for cluster in clusters['Clusters']:
    if not cluster['Encrypted']:
        print(f"Redshift cluster {cluster['ClusterIdentifier']} is not encrypted.")
```

This script will print out the identifiers of all unencrypted Redshift clusters. If no output is produced, all clusters are encrypted.
