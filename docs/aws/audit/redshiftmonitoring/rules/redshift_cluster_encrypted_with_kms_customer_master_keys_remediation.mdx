...
4. For each cluster, check the "KMS Key ID" column. If a cluster is encrypted with a KMS CMK, the ID of the key will be displayed in this column. If the column is blank or the displayed key is not a KMS CMK, then the cluster is not encrypted with a KMS CMK.
...
2. Once AWS CLI is set up, you can list all the Redshift clusters in your account using the following command:
...
3. For each cluster identifier, you can check the encryption settings using the following command:

   ```
   aws redshift describe-clusters --cluster-identifier <ClusterIdentifier> --query 'Clusters[*].[KmsKeyId]' --output text
   ```

   Replace `<ClusterIdentifier>` with the actual cluster identifier. This command will return the KMS key ID used for encryption. If the output is `None`, it means the cluster is not encrypted with a KMS CMK.

4. To automate the process, you can use a Python script that uses the boto3 library to interact with AWS. The script would list all Redshift clusters, check the encryption settings for each cluster, and print out the clusters that are not encrypted with a KMS CMK. Here is a basic example:
   ...
   This script will print out the identifiers of the clusters that are not encrypted with a KMS CMK.
...
4. Use the `describe_clusters` method to retrieve information about all Redshift clusters. Check the `KmsKeyId` attribute for each cluster. If it's `None`, the cluster is not encrypted with a KMS CMK.

```python
clusters = redshift.describe_clusters()

for cluster in clusters['Clusters']:
    if cluster['KmsKeyId'] is None:
        print(f"Cluster {cluster['ClusterIdentifier']} is not encrypted with a KMS CMK.")
```
