...
2. Once the AWS CLI is set up, you can list all the Redshift clusters in your account using the following command:
   ...
   ```
   This command will return the identifiers of all Redshift clusters in your account.
...
3. For each cluster, you can check if the User Activity Logging is enabled or not using the following command:
   ...
   ```
   Replace `<your-cluster-identifier>` with the identifier of your Redshift cluster. This command will return the logging status of the specified Redshift cluster.

4. In the output of the above command, look for the "LoggingEnabled" field. If its value is "true", it means that the User Activity Logging is enabled for the cluster. If its value is "false", it means that the User Activity Logging is not enabled for the cluster.

#### Using Python

To check if Redshift User Activity Logging is enabled in Redshift using Python scripts, you can use the Boto3 library, which allows you to write software that makes use of services like Amazon S3, Amazon EC2, and others. Here are the steps:
...
2. **Retrieve the list of Redshift clusters:**
...
3. **Iterate over the clusters and check the logging status:**
...
for cluster in response['Clusters']:
    try:
        ...
        print(f"Error retrieving logging status: {e}")
```

4. **Check if the logging is enabled and print the result:**
...
This script will print out whether User Activity Logging is enabled for each Redshift cluster in the specified region. If there's an error retrieving the logging status for a cluster, it will print an error message.
