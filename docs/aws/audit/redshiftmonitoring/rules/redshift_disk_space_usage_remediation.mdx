...
4. In the cluster details page, navigate to the "Performance" tab. Here, you can see the "Disk Space" section which shows the percentage of disk space used. If the disk usage is high, it indicates that the cluster needs to be scaled.
...
3. Check disk usage: For each cluster, check the 'TotalStorageCapacity' and 'UsedStorageCapacity' fields in the JSON output. These fields represent the total disk capacity and the used disk capacity of the cluster respectively. 
...
4. Calculate disk usage percentage: For each cluster, calculate the disk usage percentage by dividing the used storage capacity by the total storage capacity and multiplying the result by 100. If the disk usage percentage is high (typically above 80-90%), the cluster may need to be scaled. This can be done using a Python script to parse the JSON output and perform the calculation.

#### Using Python

1. Install the necessary Python libraries: Before you start, make sure you have the necessary Python libraries installed. You will need `boto3` for AWS SDK for Python and `psycopg2` for PostgreSQL database adapter. You can install them using pip:

   ```python
   pip install boto3 psycopg2-binary
   ```

2. Establish a connection to Redshift: Use the `boto3` library to establish a connection to your Redshift cluster. You will need your AWS access key, secret access key, and the region your Redshift cluster is located in.
...
3. Query the disk usage: Once you have a connection to Redshift, you can query the disk usage of your clusters. You can use the `describe_clusters` method to get information about your clusters, including the `TotalStorageCapacity` and `UsedStorageCapacity`.

   ```python
   clusters = redshift.describe_clusters()
   for cluster in clusters['Clusters']:
       total_storage = cluster['TotalStorageCapacity']
       used_storage = cluster['UsedStorageCapacity']
       print(f"Cluster: {cluster['ClusterIdentifier']}, Total Storage: {total_storage}, Used Storage: {used_storage}")
   ```

4. Check if disk usage is high: Now that you have the total and used storage capacity, you can check if the disk usage is high. For example, you might consider disk usage to be high if more than 80% of the total storage capacity is used.

   ```python
   for cluster in clusters['Clusters']:
       total_storage = cluster['TotalStorageCapacity']
       used_storage = cluster['UsedStorageCapacity']
       disk_usage = (used_storage / total_storage) * 100
       if disk_usage > 80:
           print(f"Cluster: {cluster['ClusterIdentifier']} has high disk usage: {disk_usage}%")
   ```
   
Remember to replace `'YOUR_ACCESS_KEY'`, `'YOUR_SECRET_KEY'`, and `'YOUR_REGION'` with your actual AWS access key, secret access key, and region.
