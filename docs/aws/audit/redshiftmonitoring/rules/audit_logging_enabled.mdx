...
2. Navigate to the Redshift dashboard by selecting "Services" from the top menu and then selecting "Redshift" under the "Database" category.
...
3. For each cluster identifier, you can check if logging is enabled using the following command:

   ```
   aws redshift describe-logging-status --cluster-identifier <your-cluster-identifier>
   ```

   Replace `<your-cluster-identifier>` with the actual cluster identifier. This command will return the logging status of the specified Redshift cluster.

4. If the "LoggingEnabled" field in the output is set to "false", it means that logging is not enabled for that Redshift cluster. If it's set to "true", logging is enabled. 

   Note: You can automate these steps using a Python script with the boto3 library, which is the Amazon Web Services (AWS) SDK for Python. This allows Python developers to write software that makes use of services like Amazon S3, Amazon EC2, and others.
...
3. Create a Redshift client using the session details.

```python
redshift_client = session.client('redshift')
```

4. Now, list all the Redshift clusters and check if logging is enabled for each cluster. If logging is not enabled, print the cluster identifier.

```python
def check_redshift_logging():
    try:
        clusters = redshift_client.describe_clusters()
        for cluster in clusters['Clusters']:
            if not cluster['LoggingEnabled']:
                print(f"Logging is not enabled for the cluster: {cluster['ClusterIdentifier']}")
    except (BotoCoreError, ClientError) as error:
        print(f"An error occurred: {error}")

check_redshift_logging()
```

This script will list all the Redshift clusters for which logging is not enabled. Please replace 'YOUR_ACCESS_KEY', 'YOUR_SECRET_KEY', and 'YOUR_REGION_NAME' with your actual AWS access key, secret key, and region name.
