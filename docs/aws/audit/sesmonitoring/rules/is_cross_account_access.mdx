...
3. In the SMTP Settings section, check the "IAM policies" associated with your SES identities.

4. Review the IAM policies and look for any "Principal" elements that include AWS account IDs other than your own. If you find any such policies, it means your SES identities are allowing cross-account access.

#### Using CLI

1. Install and configure AWS CLI: Before you can start using AWS CLI, you need to install it on your local machine. You can download it from the official AWS website. After installation, you need to configure it with your AWS account credentials. You can do this by running the command `aws configure` and then entering your AWS Access Key ID, Secret Access Key, Default region name, and Default output format when prompted.

2. List all SES identities: The first step to check if SES identities allow cross-account access is to list all the identities. You can do this by running the following command: `aws ses list-identities --region your-region-name`. Replace 'your-region-name' with the name of your AWS region.

3. Check the policy of each identity: For each identity listed in the previous step, you need to check its policy to see if it allows cross-account access. You can do this by running the following command: `aws ses get-identity-policies --identity "your-identity-name" --region your-region-name`. Replace 'your-identity-name' with the name of your SES identity and 'your-region-name' with the name of your AWS region.

4. Analyze the policy: The previous command will return the policy of the specified identity in JSON format. You need to analyze this policy to see if it allows cross-account access. If the policy includes a statement with the "Effect" field set to "Allow" and the "Principal" field set to a different AWS account ID, then the identity allows cross-account access.
...
3. Now, create a client for 'ses' which stands for Simple Email Service.

```python
ses_client = session.client('ses')
```

4. Now, we will list all the identities (email addresses and domains) that are verified for the current AWS account. Then, for each identity, we will get the identity policies and check if any policy allows cross-account access. Here's a sample script:

```python
identities = ses_client.list_identities()

for identity in identities['Identities']:
    policies = ses_client.get_identity_policies(
        Identity=identity,
        PolicyNames=['default']
    )
    default_policy = policies['Policies']['default']
    if 'AWS:' in default_policy:
        print(f"Identity {identity} allows cross-account access.")
```

This script will print out all the identities that allow cross-account access. If no such identities are found, it will not print anything.

Please replace `'YOUR_ACCESS_KEY'`, `'YOUR_SECRET_KEY'`, and `'YOUR_REGION'` with your actual AWS access key, secret key, and region. Also, please note that this script only checks the default policy of each identity. If you have custom policies, you might need to modify the script to check those policies as well.
