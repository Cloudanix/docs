...
2. In the navigation pane, choose "Email Addresses" under "Identity Management". This will display a list of all the email addresses that are verified for sending email through Amazon SES.

3. Check the list of verified email addresses. If any of the email addresses are not supposed to be exposed or used for sending emails, it indicates a misconfiguration.

4. For further investigation, you can select a specific email address. In the details pane, you can see more information about the email address, such as the verification status and the DKIM status. If the verification status is "verified" and the email address is not supposed to be exposed, it is a misconfiguration.
...
3. Check the policy of each identity: For each identity returned in the previous step, you can check its policy by running the following command:
   ```
   aws ses get-identity-policies --identity "IdentityName"
   ```
   Replace "IdentityName" with the actual identity name. This command will return the policy of the specified identity.

4. Analyze the policy: The last step is to analyze the returned policy. If the policy allows actions to be performed by any AWS user (i.e., if it includes a statement with "Effect": "Allow" and "Principal": "*"), then the SES identity is exposed.
...
3. Now, create a client for the 'ses' service and call the `list_identities()` method to get a list of all identities. Here's how you can do it:
...
4. Finally, iterate over the identities and check if they are exposed. You can do this by calling the `get_identity_policies()` method for each identity and checking if the policy allows public access. Here's a sample script:

```python
for identity in identities:
    policy = ses_client.get_identity_policies(
        Identity=identity,
        PolicyNames=[
            'MyPolicy',
        ]
    )
    if 'Statement' in policy['Policies']['MyPolicy']:
        for statement in policy['Policies']['MyPolicy']['Statement']:
            if statement['Effect'] == 'Allow' and 'Principal' in statement and statement['Principal'] == '*':
                print(f'Identity {identity} is exposed.')
```

This script will print out all identities that are exposed. Please replace 'MyPolicy' with the actual policy name you want to check.
