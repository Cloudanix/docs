...
4. In the details pane, look for the "DKIM" section. If DKIM is enabled, the status will be "Enabled". If it's not, the status will be "Disabled" or "Pending". 

Note: If the status is "Pending", it means that the DKIM setup process has been initiated but is not yet complete. This could be due to the DNS records for DKIM not being correctly configured in your DNS provider.
...
2. List all the identities: Use the following AWS CLI command to list all the identities (email addresses and/or domains) that are associated with your AWS account.

   ```
   aws ses list-identities
   ```

   This command will return a list of all identities.

3. Check DKIM attributes: For each identity, you can check the DKIM attributes using the following command:

   ```
   aws ses get-identity-dkim-attributes --identities 'identity1' 'identity2'
   ```

   Replace 'identity1' 'identity2' with the identities you got from the previous step. This command will return the DKIM attributes for each identity.

4. Check if DKIM is enabled: In the output of the previous command, look for the 'DkimEnabled' attribute. If its value is 'true', then DKIM signing is enabled for that identity. If its value is 'false', then DKIM signing is not enabled.
...
3. Use the `ses` client from boto3 to get the list of identities (email addresses and/or domains):
...
4. For each identity, check the DKIM attributes. If DKIM is not enabled, the `DkimEnabled` attribute will be `False`:

```python
for identity in response['Identities']:
    attributes = ses_client.get_identity_dkim_attributes(Identities=[identity])
    for attr in attributes['DkimAttributes']:
        if not attributes['DkimAttributes'][attr]['DkimEnabled']:
            print(f"DKIM is not enabled for {attr}")
```

This script will print out all the identities (email addresses and/or domains) for which DKIM is not enabled.
