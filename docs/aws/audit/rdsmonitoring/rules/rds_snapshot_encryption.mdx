...
3. In the navigation pane, click on "Snapshots". This will display a list of all your DB snapshots.
4. Select the snapshot you want to check. In the details section, look for the "KMS Key ID" field. If this field is not empty, it means that the snapshot is encrypted. If it is empty, the snapshot is not encrypted.
...
3. Check the encryption status: For each RDS instance, check the 'StorageEncrypted' field in the JSON output. If the value of this field is 'false', then the RDS instance is not encrypted. You can use the following command to filter out the instances that are not encrypted:

   ```
   aws rds describe-db-instances --query 'DBInstances[?StorageEncrypted==`false`]'
   ```

4. Check the KMS key used for encryption: For the RDS instances that are encrypted, you can check the KMS key used for encryption by looking at the 'KmsKeyId' field in the JSON output. If this field is not present or if the value is not a valid KMS key ID, then the encryption is not properly configured. You can use the following command to filter out the instances with misconfigured encryption:

   ```
   aws rds describe-db-instances --query 'DBInstances[?KmsKeyId==`null`]'
   ```
   
Please note that these commands will only detect the misconfiguration. To fix the issue, you will need to enable the snapshot encryption feature for your RDS instances.
...
```
3. Create a client for RDS and get all DB snapshots:

```python
rds = session.client('rds')
snapshots = rds.describe_db_snapshots()
```
4. Check if the snapshots are encrypted:
...
```
This script will print out the identifiers of all unencrypted RDS snapshots. If no output is produced, it means all snapshots are encrypted.
