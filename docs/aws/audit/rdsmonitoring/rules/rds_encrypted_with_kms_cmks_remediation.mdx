...
To prevent the use of AWS-managed keys in RDS and ensure the use of Customer-Managed Keys (CMKs) using the AWS Management Console, follow these steps:

1. **Navigate to RDS Dashboard:**
   - Open the AWS Management Console.
   - In the search bar, type "RDS" and select "RDS" from the dropdown to go to the RDS Dashboard.

2. **Modify or Create a Database Instance:**
   - To modify an existing database instance, select the database instance you want to modify.
   - Click on the "Modify" button.
   - To create a new database instance, click on the "Create database" button.

3. **Configure Encryption Settings:**
   - In the "Configuration" section, look for the "Encryption" settings.
   - Ensure that the "Enable Encryption" option is selected.
   - From the "Master Key" dropdown, select your Customer-Managed Key (CMK) from the list of available keys. If you do not have a CMK, you will need to create one in the AWS Key Management Service (KMS) before proceeding.

4. **Save Changes:**
   - For modifying an existing instance, scroll down and click the "Continue" button, review the changes, and then click "Modify DB Instance."
   - For creating a new instance, continue with the rest of the database configuration and click "Create database" at the end.

By following these steps, you ensure that your RDS instances use Customer-Managed Keys (CMKs) instead of AWS-managed keys for encryption.
</Accordion>

<Accordion title='Using CLI'>
To ensure that you are using Customer-Managed Keys (CMKs) instead of AWS-managed Keys for Amazon RDS, you can follow these steps using the AWS CLI:

1. **Create a Customer-Managed Key (CMK) in AWS KMS:**
   First, you need to create a CMK in AWS Key Management Service (KMS). This key will be used to encrypt your RDS instances.

   ```sh
   aws kms create-key --description "My RDS CMK" --key-usage ENCRYPT_DECRYPT --origin AWS_KMS
   ```

2. **Create an Alias for the CMK:**
   It's a good practice to create an alias for your CMK to make it easier to reference.

   ```sh
   aws kms create-alias --alias-name alias/my-rds-cmk --target-key-id <your-key-id>
   ```

   Replace `<your-key-id>` with the Key ID returned from the previous command.

3. **Launch a New RDS Instance with the CMK:**
   When creating a new RDS instance, specify the CMK you created for encryption.
   ...
   Replace `arn:aws:kms:region:account-id:key/key-id` with the ARN of your CMK.

4. **Modify an Existing RDS Instance to Use the CMK:**
   If you have an existing RDS instance, you can modify it to use the CMK. Note that this requires a snapshot and restoration process.
   ...
   Again, replace `arn:aws:kms:region:account-id:key/key-id` with the ARN of your CMK.

By following these steps, you can ensure that your RDS instances are using Customer-Managed Keys instead of AWS-managed Keys for encryption.
</Accordion>

<Accordion title='Using Python'>
To prevent the use of AWS-managed keys in RDS and ensure that customer-managed keys are used instead, you can follow these steps using Python scripts with the Boto3 library:
...
3. **Create a Customer-Managed Key (CMK)**:
   Use the AWS Key Management Service (KMS) to create a customer-managed key. This key will be used to encrypt your RDS instances.
...
4. **Modify RDS Instance to Use Customer-Managed Key**:
   Modify your RDS instance to use the customer-managed key for encryption.
   ...
   # Modify the RDS instance to use the customer-managed key
   ...
   print(f"RDS instance modified to use CMK: {response['DBInstance']['DBInstanceIdentifier']}")
   ```

By following these steps, you ensure that your RDS instances use customer-managed keys instead of AWS-managed keys for encryption. This approach provides you with more control over the encryption keys and enhances the security of your data.
</Accordion>
