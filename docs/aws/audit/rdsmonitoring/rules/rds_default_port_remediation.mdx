...
To prevent RDS Database Instances from using default ports in AWS using the AWS Management Console, follow these steps:

1. **Navigate to RDS Dashboard:**
   - Sign in to the AWS Management Console.
   - In the Services menu, select "RDS" to go to the Amazon RDS dashboard.

2. **Modify the Database Instance:**
   - In the RDS dashboard, click on "Databases" in the left-hand menu.
   - Select the database instance you want to modify by clicking on its identifier.

3. **Edit Database Configuration:**
   - In the database instance details page, click on the "Modify" button.
   - Scroll down to the "Connectivity" section.

4. **Change the Port:**
   - In the "Port" field, enter a non-default port number (e.g., instead of the default port 3306 for MySQL, use a different port like 3307).
...
By following these steps, you can ensure that your RDS database instances do not use default ports, enhancing the security of your database.
</Accordion>

<Accordion title='Using CLI'>
To prevent RDS Database Instances from using default ports in AWS using the AWS CLI, you can follow these steps:

1. **Describe the RDS Instances**:
   First, identify the RDS instances and their current port configurations.
   ```sh
   aws rds describe-db-instances --query 'DBInstances[*].{DBInstanceIdentifier:DBInstanceIdentifier, Port:Endpoint.Port}'
   ```

2. **Modify the RDS Instance Port**:
   If an RDS instance is using the default port, modify it to use a non-default port. Replace `<db-instance-identifier>` with your actual DB instance identifier and `<new-port>` with the desired non-default port number.
   ```sh
   aws rds modify-db-instance --db-instance-identifier <db-instance-identifier> --db-port-number <new-port> --apply-immediately
   ```

3. **Verify the Changes**:
   After modifying the port, verify that the changes have been applied successfully.
   ```sh
   aws rds describe-db-instances --db-instance-identifier <db-instance-identifier> --query 'DBInstances[*].{DBInstanceIdentifier:DBInstanceIdentifier, Port:Endpoint.Port}'
   ```

4. **Update Security Groups**:
   Ensure that the associated security groups allow traffic on the new port. Replace `<security-group-id>` with your security group ID and `<new-port>` with the new port number.
   ```sh
   aws ec2 authorize-security-group-ingress --group-id <security-group-id> --protocol tcp --port <new-port> --cidr 0.0.0.0/0
   ```

By following these steps, you can prevent RDS Database Instances from using default ports using the AWS CLI.
</Accordion>

<Accordion title='Using Python'>
To prevent RDS Database Instances from using default ports in AWS using Python scripts, you can use the Boto3 library, which is the AWS SDK for Python. Here are the steps to achieve this:
...
3. **Create a Python Script**:
   Write a Python script to modify the port of your RDS instances. Below is an example script:
   ...
   # Function to modify the port of an RDS instance
   def modify_rds_instance_port(instance_identifier, new_port):
       try:
           ...
           print(f"Successfully modified the port for instance {instance_identifier} to {new_port}")
       except Exception as e:
           print(f"Error modifying the port for instance {instance_identifier}: {e}")

   # List of RDS instances and their new ports
   rds_instances = [
       {'instance_id': 'your-rds-instance-1', 'new_port': 5432},
       {'instance_id': 'your-rds-instance-2', 'new_port': 3307},
       # Add more instances as needed
   ]

   # Modify the port for each RDS instance
   for instance in rds_instances:
       modify_rds_instance_port(instance['instance_id'], instance['new_port'])
   ```

4. **Run the Script**:
   Execute the script to modify the ports of your RDS instances.
   ```bash
   python modify_rds_ports.py
   ```

### Summary:
1. Install the Boto3 library.
2. Set up AWS credentials.
3. Create a Python script to modify the RDS instance ports.
4. Run the script to apply the changes.

This script will help you prevent RDS Database Instances from using default ports by modifying them to the specified custom ports.
</Accordion>
