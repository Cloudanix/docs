...
2. In the navigation pane, choose "WebACLs". This will display a list of all the WebACLs that are currently configured in your AWS account.

3. Select the WebACL that you want to inspect. This will open the details page for that WebACL.

4. In the details page, look for the "Rules" section. This section will list all the rules that are currently associated with the selected WebACL. If there are any Rule Groups associated with the WebACL, they will be listed here. If there are no Rule Groups listed, then the WebACL does not contain any Rule Groups.
...
2. List all WebACLs: Use the `list-web-acls` command to get a list of all WebACLs in your AWS account. This command returns an array of WebACL Summary objects.
   ...
   Note: Replace "REGIONAL" with "CLOUDFRONT" if you want to list the WebACLs associated with CloudFront.

3. Get WebACL details: For each WebACL returned by the previous command, use the `get-web-acl` command to get the details of the WebACL. This command returns a WebACL object that contains the rules of the WebACL.

   ```
   aws wafv2 get-web-acl --scope REGIONAL --id <WebACLId> --name <WebACLName> --region <Region>
   ```

   Replace `<WebACLId>`, `<WebACLName>`, and `<Region>` with the ID, name, and region of the WebACL respectively.

4. Check for Rule Group: In the output of the `get-web-acl` command, look for the `Rules` field. This field contains an array of Rule objects. Each Rule object has a `Statement` field that can contain a `RuleGroupReferenceStatement` field if the rule is a rule group. If the `RuleGroupReferenceStatement` field is present, then the WebACL contains a rule group. If not, then it does not contain a rule group.
...
2. **Create a Python script to list all WebACLs:**
   ...
   print(response)
   ```
   This script will print out all the WebACLs in your account.

3. **Create a Python script to get the details of each WebACL:**
   Use the `get_web_acl` method to retrieve the details of each WebACL. Here's a sample script:
   ```python
   import boto3
   ...
   print(response)
   ```
   Replace 'web_acl_id' and 'web_acl_name' with the actual ID and name of the WebACL you want to check. This script will print out the details of the specified WebACL.

4. **Check if the WebACL contains Rule Group or Groups:**
   In the response from the `get_web_acl` method, look for the 'Rules' field. If the WebACL contains a Rule Group or Groups, they will be listed in this field. Here's a sample script:
   ```python
   import boto3
   ...
   for rule in rules:
       if 'RuleGroupReferenceStatement' in rule['Statement']:
           print('WebACL contains Rule Group:', rule['Name'])
   ```
   This script will print out the names of the Rule Groups contained in the specified WebACL. If no Rule Groups are found, it will not print anything.
