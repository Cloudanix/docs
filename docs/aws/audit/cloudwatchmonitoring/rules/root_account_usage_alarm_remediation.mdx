...
##### Create the necessary metric filter and the CloudWatch alarm that will fire and send email notifications whenever an AWS Root Account is used
...
- Step 3: On the Define Logs Metric Filter page, paste the following pattern inside the Filter Pattern box and type the following patter: \{ $.userIdentity.type = "Root" && $.userIdentity.invokedBy NOT EXISTS && $.eventType != "AwsServiceEvent" \}
...
- Step 5: On the Create Metric Filter and Assign a Metric page, perform the following:
  A: In the Filter Name box, enter a unique name for the new filter, e.g. RootAccountUsage.

  B: In the Metric Namespace box, type CloudTrailMetrics.

  C: In the Metric Name box, type RootAccountUsageEventCount for the metric identifier.
...
- Step 8: In the Create Alarm dialog box, provide the following information:
  ...
  B: Under Whenever: Metric Name, select >= (greater than or equal to) from the is dropdown list and enter 1 as the threshold value in the box next to the dropdown list to trigger the alarm every time Root Account is used.
