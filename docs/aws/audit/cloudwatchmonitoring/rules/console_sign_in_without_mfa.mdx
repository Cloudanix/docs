...
Note: This alarm will only trigger if AWS CloudTrail is configured to log console sign-in events and if these events are integrated with CloudWatch. If not, you will need to set up these configurations first.
...
2. Use the following AWS CLI command to list all the users in your AWS account:
   ...
   This command will return a list of all the user names in your AWS account.

3. For each user, you can check if they have MFA enabled by using the following AWS CLI command:

   ```
   aws iam list-mfa-devices --user-name <username>
   ```

   Replace `<username>` with the name of the user you want to check. This command will return a list of all the MFA devices associated with the specified user. If the list is empty, it means that the user does not have MFA enabled.

4. To monitor AWS Console Sign In Without MFA, you can use AWS CloudWatch. You can create a CloudWatch alarm that triggers whenever a user signs in to the AWS Console without MFA. Use the following AWS CLI command to create a CloudWatch alarm:

   ```
   aws cloudwatch put-metric-alarm --alarm-name "SignInWithoutMFA" --metric-name "ConsoleSignInWithoutMFA" --namespace "AWS/SignIn" --statistic "SampleCount" --period 300 --threshold 1 --comparison-operator "GreaterThanOrEqualToThreshold" --evaluation-periods 1 --alarm-actions <arn_of_sns_topic>
   ```

   Replace `<arn_of_sns_topic>` with the ARN of the SNS topic where you want to receive the alarm notifications. This command will create a CloudWatch alarm that triggers whenever a user signs in to the AWS Console without MFA.
...
1. Install and configure AWS SDK for Python (Boto3):
   ...
   pip install boto3
   ```
   After installing Boto3, you need to configure it. You can use the AWS CLI to configure Boto3:
   ```
   aws configure
   ```
...
2. Use Boto3 to interact with AWS CloudWatch:
   You can use Boto3 to interact with AWS CloudWatch and get the login events. Here is a sample script:
   ```python
   import boto3
   ...
   print(get_login_events())
   ```
   This script will print the login events from the specified log group.

3. Parse the login events to find sign-ins without MFA:
   You can parse the login events to find sign-ins without MFA. Here is a sample script:
   ```python
   import json

   def parse_login_events(events):
       for event in events['Datapoints']:
           event_data = json.loads(event['SampleCount'])
           if 'userIdentity' in event_data and 'sessionContext' in event_data['userIdentity']:
               if 'attributes' in event_data['userIdentity']['sessionContext'] and 'mfaAuthenticated' in event_data['userIdentity']['sessionContext']['attributes']:
                   if event_data['userIdentity']['sessionContext']['attributes']['mfaAuthenticated'] == 'false':
                       print('Sign-in without MFA detected: ', event_data)

   parse_login_events(get_login_events())
   ```
   This script will print the details of the sign-ins without MFA.

4. Schedule the script to run periodically:
   You can schedule the script to run periodically to monitor the sign-ins without MFA. You can use cron jobs in Unix-based systems or Task Scheduler in Windows to schedule the script.
