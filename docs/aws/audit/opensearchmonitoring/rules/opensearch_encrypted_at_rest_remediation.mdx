...
2. Navigate to the OpenSearch Service dashboard. You can do this by typing "OpenSearch Service" in the search bar and selecting it from the dropdown menu.
3. In the OpenSearch Service dashboard, you will see a list of all your OpenSearch domains. Click on the domain name that you want to check.
4. In the domain dashboard, navigate to the "Encryption" section. Here, you can see if encryption at rest is enabled or not. If it is enabled, you will see the details of the KMS key used for encryption. If it is not enabled, the section will say "Encryption at rest is disabled".

#### Using CLI

1. First, you need to install and configure AWS CLI on your local machine. Make sure you have the necessary permissions to access the OpenSearch Service Domains.

2. Use the following AWS CLI command to list all the OpenSearch domains:

   ```
   aws es list-domain-names
   ```
   This command will return a list of all the OpenSearch (Elasticsearch) domains.
...
3. For each domain, you can describe the domain configuration using the following command:
   ...
   ```
   Replace `<your-domain-name>` with the name of your domain. This command will return the configuration details of the specified domain.

4. In the output of the above command, look for the `EncryptionAtRestOptions` field. If the `Enabled` field under `EncryptionAtRestOptions` is `false`, then Encryption at Rest is not enabled for that OpenSearch Service Domain.
...
3. Write the Python script: Now you can write a Python script that uses the boto3 library to check if Encryption At Rest is enabled for OpenSearch Service Domains. Here is a simple script that does this:
   ...
   def check_encryption_at_rest():
       ...
       for domain in domains['DomainNames']:
           ...
           encryption_at_rest_options = domain_config['DomainStatus']['EncryptionAtRestOptions']
           if encryption_at_rest_options['Enabled']:
               print(f"Encryption at rest is enabled for domain {domain_name}")
           else:
               print(f"Encryption at rest is NOT enabled for domain {domain_name}")

   if __name__ == "__main__":
       check_encryption_at_rest()
   ```

4. Run the Python script: Finally, you can run the Python script. It will print out whether Encryption At Rest is enabled for each OpenSearch Service Domain. If Encryption At Rest is not enabled for a domain, it will print a warning message.
