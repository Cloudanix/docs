...
3. Under the "Settings" section, you will find the "Providers" tab. Click on it.

4. Here, you can check if AWS Shield is enabled or not. If it is enabled, it will be listed under the "Enabled providers" section. If it is not, it will be listed under the "Disabled providers" section.
...
3. Check if AWS Security Hub is enabled: For each account returned in the previous step, run the following command to check if AWS Security Hub is enabled:
   ```
   aws securityhub get-master-account --region <region-name>
   ```
   Replace `<region-name>` with the name of the region you want to check. This command will return information about the master account for AWS Security Hub.

4. Analyze the output: If AWS Security Hub is enabled, the output of the previous command will include the ARN of the master account. If it is not enabled, the command will return an error message. 

Remember to replace `<region-name>` with the actual region name in the above commands.
...
3. Create a Python Script: Now, you can create a Python script to check if AWS Security Hub is enabled or not. Here is a simple script that uses Boto3 to check the status of AWS Security Hub:
...
def check_security_hub_status():
    ...
    try:
        ...
        print("Error occurred while checking Security Hub status: ", e)

check_security_hub_status()
```

4. Run the Python Script: Finally, you can run the Python script. If Security Hub is enabled, it will print "Security Hub is enabled." If it is not enabled, it will print "Security Hub is not enabled." If there is an error while checking the status, it will print the error message.

Please note that this script assumes that you have the necessary permissions to call the 'get_enabled_standards' API. If you don't have the necessary permissions, you will need to update your IAM policy to include 'securityhub:GetEnabledStandards' permission.
