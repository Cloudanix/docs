...
3. In the Route 53 dashboard, select "Hosted zones" from the left-hand menu. This will display a list of all the hosted zones associated with your AWS account.
4. For each hosted zone, click on the domain name to view its record sets. Check if there is a record set with the type "SPF". If there is no SPF record present, then it indicates a misconfiguration.
...
4. Now, you need to check if there is a record set of type "SPF" in the list of record sets. You can do this by looking at the "Type" field of each record set. If there is no record set of type "SPF", then the hosted zone does not have a Sender Policy Framework record present.
...
2. Import the necessary Python modules and initialize the AWS client: You will need to import the Boto3 module and initialize the AWS client to interact with AWS services.
...
3. Fetch the list of hosted zones: You can use the `list_hosted_zones()` function to fetch the list of hosted zones.
...
4. Check for SPF records: For each hosted zone, you can fetch the list of resource record sets and check if there is an SPF record present. If not, the hosted zone is misconfigured.
   ...
   for zone in hosted_zones:
       ...
       spf_record_present = any(record_set for record_set in record_sets if record_set['Type'] == 'SPF')

       if not spf_record_present:
           print(f"Hosted zone {zone_id} is misconfigured. No SPF record present.")
   ```

This script will print out the IDs of all hosted zones that do not have an SPF record present, indicating that they are misconfigured.
