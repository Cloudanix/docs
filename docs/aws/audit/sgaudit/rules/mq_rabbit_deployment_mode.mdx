...
2. Once the AWS CLI is set up, you can use the "describe-security-groups" command to list all the security groups and their details. The command is as follows:
   ...
   This command will return a JSON output with all the details of the security groups.

3. To check if the MQ Rabbit has a deployment mode, you need to look for the "GroupName" field in the output. If the "GroupName" is "MQ Rabbit", then it means that the MQ Rabbit has a deployment mode.

4. To further verify, you can use the "describe-instances" command to list all the instances and their details. The command is as follows:

   ```
   aws ec2 describe-instances
   ```

   In the output, look for the "SecurityGroups" field. If the "GroupName" is "MQ Rabbit", then it confirms that the MQ Rabbit has a deployment mode.
...
2. **Configure AWS Credentials:**
   Before you can begin using Boto3, you need to set up authentication credentials. You can do this in several ways, but the easiest way is to use the AWS CLI tool to configure everything. If you've already done this, you can skip this step.
   ```
   aws configure
   ```
...
3. **Python Script to Check Security Groups:**
   Here is a simple Python script that uses Boto3 to retrieve all security groups and check if MQ Rabbit has a deployment mode.

   ```python
   import boto3

   def check_mq_rabbit_deployment_mode():
       ec2 = boto3.resource('ec2')
       for security_group in ec2.security_groups.all():
           if 'MQ Rabbit' in security_group.group_name:
               print(f"Security Group: {security_group.group_name}")
               for permission in security_group.ip_permissions:
                   if 'FromPort' in permission and 'ToPort' in permission:
                       if permission['FromPort'] == 5671 and permission['ToPort'] == 5672:
                           print("MQ Rabbit has a deployment mode.")
                       else:
                           print("MQ Rabbit does not have a deployment mode.")

   if __name__ == "__main__":
       check_mq_rabbit_deployment_mode()
   ```
   This script will print out whether MQ Rabbit has a deployment mode based on the security group settings.

4. **Run the Python Script:**
   Finally, you can run the Python script from your terminal:
   ```
   python check_mq_rabbit_deployment_mode.py
   ```
   This will print out the status of MQ Rabbit's deployment mode in each security group.

Please note that this script assumes that the deployment mode of MQ Rabbit is determined by whether ports 5671 and 5672 are open. You may need to adjust the script based on your specific criteria for what constitutes a "deployment mode" for MQ Rabbit.
