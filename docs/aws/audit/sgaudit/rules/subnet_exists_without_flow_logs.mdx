...
4. In the details pane, on the "Flow logs" tab, check if there are any flow logs associated with the VPC. If there are no flow logs, then the Flow Logs are not enabled on the subnet.

5. If you want to check for a specific subnet, go back to the navigation pane and choose "Subnets". Select the subnet you want to check and repeat the process in the details pane, on the "Flow logs" tab.
...
2. **List All VPCs**: The first step to check if Flow Logs are enabled on a subnet is to list all the VPCs in your account. You can do this by running the command `aws ec2 describe-vpcs`. This will return a list of all the VPCs in your account.

3. **List All Subnets in a VPC**: Once you have the VPC ID, you can list all the subnets in that VPC by running the command `aws ec2 describe-subnets --filters "Name=vpc-id,Values=<VPC_ID>"`. Replace `<VPC_ID>` with the ID of the VPC you are interested in. This will return a list of all the subnets in that VPC.

4. **Check if Flow Logs are Enabled on a Subnet**: To check if Flow Logs are enabled on a subnet, you need to describe the flow logs for that subnet by running the command `aws ec2 describe-flow-logs --filter "Name=resource-id,Values=<SUBNET_ID>"`. Replace `<SUBNET_ID>` with the ID of the subnet you are interested in. If Flow Logs are enabled on the subnet, this command will return information about the Flow Logs. If Flow Logs are not enabled, this command will return an empty result.
...
3. Get a list of all subnets:
   Use the `describe_subnets()` function to get a list of all subnets.
...
4. Check if Flow Logs are enabled:
   For each subnet, use the `describe_flow_logs()` function to check if Flow Logs are enabled. If the length of the response is 0, then Flow Logs are not enabled for that subnet.

   ```python
   for subnet in subnets:
       flow_logs = ec2.meta.client.describe_flow_logs(
           Filter=[
               {
                   'Name': 'resource-id',
                   'Values': [
...
This script will print out the IDs of all subnets for which Flow Logs are not enabled.
