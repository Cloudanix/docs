...
A principal (IAM user, group, or role) is flagged when it has actions matching infrastructure modification patterns (using wildcard matching) across the following services:
...
**Action patterns per service:** `Create*`, `Update*`, `Delete*`, `Put*`, `Associate*`, `Disassociate*`, `Modify*`, `Remove*`.

- For EC2, the following additional patterns are also evaluated: `Authorize*`, `Revoke*`.
