...
A principal (IAM user, group, or role) is flagged when it has permissions that allow creating, updating, deleting, or modifying resources across the following AWS services:

**Services covered:** API Gateway, Athena, Auto Scaling, Bedrock, Cassandra, CloudFormation, CloudFront, DynamoDB, EBS, EC2, ElastiCache, Elastic Beanstalk, EFS, ELB, Elasticsearch, Kinesis, Lambda, RDS, Redshift, S3, SES, SNS, SQS, SSM, Storage Gateway, WAF, WAFv2.

**Actions checked:** Any action that creates, updates, deletes, modifies, associates, disassociates, or removes resources within the above services.

- For EC2, actions that authorize or revoke access are also checked.
