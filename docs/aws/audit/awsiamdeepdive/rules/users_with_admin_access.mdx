...
`isAdmin` — Indicates that the principal has administrator access. There should be a minimum number of admins in your account to reduce risk.
...
A principal is flagged when it has the `AdministratorAccess` policy attached.

**Note on threshold:** As a standalone check, this rule only flags when the number of admin principals exceeds the allowed limit (default: 2). When used for labeling, all principals with `AdministratorAccess` are labeled as `isAdmin`.
