...
A principal is flagged when it has the AWS managed policy named `AdministratorAccess` attached to it.

**Note on threshold:** When this rule runs as a standalone check (from the handler), it only flags if the number of admin principals exceeds `maxAllowedIAMAdmins` (configured as 2). However, when used for labeling principals in the relationship mapping, **all** principals with `AdministratorAccess` are labeled as `isAdmin` (threshold = 0).
