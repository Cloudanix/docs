...
Indicates that the principal has administrator access. There should be a minimum number of admins in your account to reduce risk.
...
A principal (user) is flagged when it has the AWS managed `AdministratorAccess` policy attached, which grants full access to all AWS services and resources.

**Note on threshold:** As a standalone check, this rule only flags when the total number of admin principals in your account exceeds the allowed limit (default: 2).
