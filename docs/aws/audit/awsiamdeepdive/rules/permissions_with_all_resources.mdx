...
Indicates that the principal (IAM user, group, or role) has access to all resources instead of specific ones.
...
A principal (IAM user, group, or role) is flagged when it has a policy that grants access to all resources (`resource = *`) instead of being limited to specific ones.
