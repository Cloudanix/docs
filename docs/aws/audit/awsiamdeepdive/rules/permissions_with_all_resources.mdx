...
`hasAccessToAllResources` — Indicates that the principal (IAM user, group, or role) has access to all resources instead of specific ones.
...
A principal (IAM user, group, or role) is flagged when it has a policy statement where the resource field contains `*`, indicating unrestricted access to all resources.
