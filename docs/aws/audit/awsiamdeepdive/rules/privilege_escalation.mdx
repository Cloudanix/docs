...
Indicates that the IAM user or role has permissions that could be leveraged to escalate privileges.
...
A principal (IAM user or role) is flagged when it has permissions that could be combined to gain higher access than originally intended. The table below shows the known escalation paths and the permission combinations that trigger this rule:

| Escalation Path | Required Permissions |
| --- | --- |
| Direct policy manipulation | Any of: `iam:CreatePolicyVersion`, `iam:SetDefaultPolicyVersion`, `iam:CreateAccessKey`, `iam:CreateLoginProfile`, `iam:UpdateLoginProfile`, `iam:AttachUserPolicy`, `iam:AttachGroupPolicy`, `iam:PutUserPolicy`, `iam:PutGroupPolicy`, `iam:AddUserToGroup`, `lambda:UpdateFunctionCode` |
| EC2 + PassRole | `ec2:RunInstances` **and** `iam:PassRole` |
| Lambda/SSM + PassRole | Any of (`lambda:CreateFunction`, `lambda:InvokeFunction`, `lambda:CreateEventSourceMapping`, `ssm:SendCommand`, `ssm:StartSession`) **and** `iam:PassRole` |
| DynamoDB + PassRole | Any of (`dynamodb:PutItem`, `dynamodb:CreateTable`) **and** `iam:PassRole` |
| Glue + PassRole | Any of (`glue:CreateDevEndpoint`, `glue:UpdateDevEndpoint`) **and** `iam:PassRole` |
| CloudFormation + PassRole | Any of (`cloudformation:CreateStack`, `cloudformation:UpdateStack`) **and** `iam:PassRole` |
| Role policy + AssumeRole | Any of (`iam:UpdateAssumeRolePolicy`, `iam:PutRolePolicy`, `iam:AttachRolePolicy`) **and** `sts:AssumeRole` |
| DataPipeline + PassRole | `datapipeline:CreatePipeline` **and** `iam:PassRole` |
