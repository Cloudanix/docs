...
`hasNotActions` — Indicates that the policy has a statement using `NotAction` with an `Allow` effect, which grants access to all actions except the ones listed.
### Risk Level
...
A policy is flagged when the statement has `NotAction` defined (not null), **and** the effect is set to `Allow`.
