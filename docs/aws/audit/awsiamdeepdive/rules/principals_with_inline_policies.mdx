...
A principal (IAM user, group, or role) is flagged when it has a policy of type inline, as opposed to AWS managed or customer managed policies. Inline policies are embedded directly into a principal and are harder to manage, audit, and reuse.
