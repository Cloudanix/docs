...
`isInactive` — Indicates that the particular user has not been active recently.
...
A user is considered inactive when any of the following conditions are true:

- The `isUsed` flag is marked as `false` on the user.
- The `passwordLastUsed` timestamp is older than 90 days, **and** all access key `lastUsedDate` values are also older than 90 days (or null).
- If no password was ever set, the user is flagged when all access key `lastUsedDate` values are older than 90 days or null.
