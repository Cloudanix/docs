...
Indicates that the particular user has not been active recently.
...
A user is flagged as inactive when any of the following apply:

- AWS identifies the user as unused.
- The user's last console login — the date the password was last used to sign in to the AWS Management Console — is older than 90 days.
- If the user has never set a password, this rule checks whether all access key last-used dates are older than 90 days or never used.
