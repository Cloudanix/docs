...
Indicates a risk of data exfiltration, where an authorized person could extract information from a secured environment and share it with unauthorized parties or move it to insecure systems.
...
A principal (IAM user, group, or role) is flagged when **both** of the following are true:

- The principal has permissions that include one or more actions from the data exfiltration list below.
- Those permissions apply to all resources (`resource = *`) rather than being scoped to specific ones.
