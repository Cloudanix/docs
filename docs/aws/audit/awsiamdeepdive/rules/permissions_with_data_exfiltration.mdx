...
`hasDataExfiltrationRisk` — Indicates a risk of data exfiltration, where an authorized person could extract information from a secured environment and share it with unauthorized parties or move it to insecure systems.
...
This rule evaluates IAM principals based on the following conditions. A principal is flagged when **both** criteria are satisfied:

- The principal has an attached policy containing one or more actions from the monitored exfiltration actions listed below.
- The policy statement grants access to **all** resources (`resource = *`), indicating overly broad permissions.

### Monitored Actions

| Service | Actions |
| --- | --- |
| DynamoDB | `BatchExecuteStatement`, `BatchGetItem`, `GetItem`, `TransactGetItems` |
| EC2 (Snapshots/Volumes) | `AttachVolume`, `CopySnapshot`, `CreateReplaceRootVolumeTask`, `CreateSnapshot`, `CreateSnapshots`, `CreateTags`, `CreateVolume`, `DetachVolume`, `ModifySnapshotAttribute`, `ModifySnapshotTier`, `ModifyVolume`, `ModifyVolumeAttribute`, `ResetSnapshotAttribute`, `RestoreSnapshotFromRecycleBin`, `RestoreSnapshotTier` |
| IAM | `GetUser` |
| KMS | `Decrypt` |
| RDS | `CopyDBClusterSnapshot`, `CopyDBSnapshot`, `CreateDBClusterSnapshot`, `CreateDBInstanceReadReplica`, `CreateDBSnapshot`, `ModifyDBCluster`, `ModifyDBClusterSnapshotAttribute`, `ModifyDBInstance`, `ModifyDBSnapshot`, `ModifyDBSnapshotAttribute`, `ModifyGlobalInstance`, `Select` |
| S3 | `CopyObject`, `GetBucketTagging`, `GetObject`, `HeadBucket`, `HeadObject`, `PutBucketPolicy`, `PutObjectAcl`, `RestoreObject`, `SelectObjectContent` |
