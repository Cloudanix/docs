...
- The role is trusted by any of the following compute service principals: `ec2.amazonaws.com`, `lambda.amazonaws.com`, `ecs.amazonaws.com`, or `eks.amazonaws.com`.
- The role has actions matching `rds:CopyDBSnapshot` or `rds:CreateDBSnapshot`, **or** the policy statement applies to all resources (`resource = *`).
