...
3. Select the desired environment from the list of available environments.
4. In the environment overview page, click on the "Configuration" link in the sidebar.
5. Under the "Software" category, click on the "Edit" button.
6. In the "Log Options" section, check if the "Instance log streaming" is enabled. If it is enabled, it means that the persistent logs are enabled for the Elastic Beanstalk environment. If it is not enabled, it indicates a misconfiguration.
...
4. **Check if the 'OptionSettings' for each environment has the 'aws:elasticbeanstalk:command' namespace with the 'LogPublicationControl' option set to 'true':**
    ...
    for environment in environments['Environments']:
        ...
        for option in response['OptionSettings']:
            if option['Namespace'] == 'aws:elasticbeanstalk:command' and option['OptionName'] == 'LogPublicationControl':
                if option['Value'] != 'true':
                    print(f"Persistent logs are not enabled for environment: {environment['EnvironmentName']}")
    ```

This script will print out the names of all Elastic Beanstalk environments where persistent logs are not enabled.
