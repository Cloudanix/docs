...
4. In the lower panel, select the "Outbound" tab. This tab displays the outbound access rules for the selected security group.
5. Check the "Destination" column for any rule that allows all traffic (0.0.0.0/0) to all ports (0 - 65535). If such a rule exists, it means that unrestricted outbound access is allowed.
...
3. Check outbound rules for each security group: For each security group, you can check the outbound rules by running the following command:
   ```
   aws ec2 describe-security-groups --group-ids <Security_Group_ID> --query 'SecurityGroups[*].IpPermissionsEgress'
   ```
   Replace `<Security_Group_ID>` with the ID of the security group you want to check. This command will return a list of all outbound rules for the specified security group.

4. Detect unrestricted outbound access: If any security group has an outbound rule that allows all traffic (0.0.0.0/0) on all ports, it means that unrestricted outbound access is allowed. You can detect this by checking the output of the previous command. If the "IpRanges" field contains "0.0.0.0/0", and the "FromPort" and "ToPort" fields cover all ports (0-65535), then unrestricted outbound access is allowed.
...
3. Create a Python script: Now, you can create a Python script to check for unrestricted outbound access in EC2 security groups. Here is a simple script that lists all security groups and their outbound rules:
   ...
   for sg in security_groups:
       print(f"Security Group: {sg.group_name}")
           ...
           if 'IpRanges' in rule:
               for ip_range in rule['IpRanges']:
                   if ip_range['CidrIp'] == '0.0.0.0/0':
                       print(f"Unrestricted Outbound Access Detected: {rule}")
   ```
...
This script will print the details of any security group that has an outbound rule allowing unrestricted access.
