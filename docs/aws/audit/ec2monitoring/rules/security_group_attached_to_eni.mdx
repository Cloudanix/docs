---
slug: security_group_attached_to_eni
title: Non-Default Security Groups Should Be Attached To Elastic Network Interface
sidebar_label: Non-Default Security Groups Should Be Attached To Elastic Network Interface
---
...
4. In the details pane at the bottom, under the "Security groups" section, you can see the security groups attached to the ENI. If the attached security group is the default one, then it is a misconfiguration.
...
2. Once the AWS CLI is configured, you can list all the security groups in your AWS account by running the following command:

   ```
   aws ec2 describe-security-groups --query 'SecurityGroups[*].{Name:GroupName,ID:GroupId}'
   ```
   This command will return a list of all security groups along with their IDs.

3. Next, you can list all the Elastic Network Interfaces (ENIs) in your AWS account and the security groups attached to them by running the following command:

   ```
   aws ec2 describe-network-interfaces --query 'NetworkInterfaces[*].{ID:NetworkInterfaceId,SG:Groups}'
   ```
   This command will return a list of all ENIs along with the security groups attached to them.

4. Now, you can compare the list of security groups attached to ENIs with the list of all security groups. If there are any security groups that are not attached to any ENI, those are the non-default security groups that are not attached to any ENI.
...
4. Now, iterate over all the network interfaces in your region and check if they are attached to a non-default security group:
...
This script will print out the IDs of all ENIs that are attached to a non-default security group. If you want to check this for all regions, you can get a list of all regions using `session.get_available_regions('ec2')` and then create an EC2 resource object for each region.
