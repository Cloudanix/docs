...
3. For each security group, check the inbound rules to see if they allow unrestricted access to Elasticsearch. You can do this by using the following command:
...
4. Check the output of the previous command for any rules that allow access to port 9200 (the default port for Elasticsearch) from any IP address (0.0.0.0/0). If such a rule exists, then unrestricted Elasticsearch access is allowed.
...
def check_es_access():
    ...
    for domain in domains['DomainNames']:
        ...
        if 'Principal': {'AWS': '*'} in access_policy:
            print(f"Unrestricted Elasticsearch access is allowed in domain: {domain_name}")

check_es_access()
```

This script first lists all Elasticsearch domains. Then for each domain, it retrieves the domain configuration and checks the access policy. If the access policy allows unrestricted access (indicated by `'Principal': {'AWS': '*'})`, it prints a message.
...
This will print a message for each Elasticsearch domain that allows unrestricted access.
