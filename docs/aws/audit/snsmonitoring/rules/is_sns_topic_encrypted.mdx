...
3. In the SNS dashboard, select "Topics" from the left-hand navigation pane. This will display a list of all the SNS topics available in your AWS account.
4. For each topic in the list, check the "Encryption" column. If the column displays "Disabled" or "Not enabled", then the SNS topic is not encrypted, indicating a misconfiguration. If the column displays "Enabled" or the name of a KMS key, then the SNS topic is encrypted.

#### Using CLI

1. First, you need to list all the SNS topics in your AWS account. You can do this by using the AWS CLI command `aws sns list-topics`. This command will return a list of all SNS topics in your account.

2. Once you have the list of SNS topics, you can iterate over each topic to check its encryption status. You can do this by using the AWS CLI command `aws sns get-topic-attributes --topic-arn <TopicARN>`. Replace `<TopicARN>` with the ARN of the SNS topic you want to check. This command will return the attributes of the specified SNS topic.

3. From the returned attributes, you need to check the `KmsMasterKeyId` attribute. If this attribute is present and has a value, it means that the SNS topic is encrypted.

4. If the `KmsMasterKeyId` attribute is not present or does not have a value, it means that the SNS topic is not encrypted. You can use this information to identify the SNS topics in your AWS account that are not encrypted.
...
4. Iterate over the list of topics and check if they are encrypted. If the 'KmsMasterKeyId' attribute is not present, the topic is not encrypted:
...
for topic in response['Topics']:
    ...
    if 'KmsMasterKeyId' not in topic_attributes['Attributes']:
        print(f"The SNS Topic {topic['TopicArn']} is not encrypted.")
```

This script will print out the ARNs of all unencrypted SNS topics. You can modify it to suit your needs, for example by adding logging or sending notifications when unencrypted topics are found.
