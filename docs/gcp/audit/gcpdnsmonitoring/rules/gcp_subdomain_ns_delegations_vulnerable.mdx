---
slug: gcp_subdomain_ns_delegations_vulnerable
title: GCP Subdomain NS Delegations Vulnerable
sidebar_label: GCP Subdomain NS Delegations Vulnerable
---
...
1. Login to Google Cloud Console: First, you need to log in to your Google Cloud Console using your credentials. 

2. Navigate to Cloud DNS: Once you are logged in, navigate to the "Network Services" section and then click on "Cloud DNS". 

3. Check DNS Zones: Here, you will see a list of all your DNS zones. Click on the DNS zone that you want to check for subdomain NS delegations. 

4. Review NS Records: In the DNS zone details page, you will see a list of all the DNS records associated with that zone. Look for NS (Name Server) records that are associated with your subdomains. If there are NS records that delegate authority to external name servers, then your subdomain NS delegations might be vulnerable.
...
2. List all the managed zones: Once the SDK is authenticated, you can list all the managed zones in your GCP project using the command `gcloud dns managed-zones list`. This will give you a list of all the DNS zones in your project.

3. Get the details of a managed zone: To check the NS delegations of a specific managed zone, you need to get the details of that zone. You can do this using the command `gcloud dns record-sets list --zone=[ZONE_NAME]`. Replace `[ZONE_NAME]` with the name of the zone you want to check. This will give you a list of all the record sets in that zone.

4. Check the NS records: In the list of record sets, look for the NS records. These are the nameserver delegations for the zone. If the NS records are pointing to a subdomain of the zone, then the zone is vulnerable to subdomain takeover. If the NS records are pointing to external nameservers, then the zone is not vulnerable.

#### Using Python

1. Install the necessary Python libraries: You will need the 'dnspython' library to perform DNS queries. You can install it using pip:
```python
pip install dnspython
```

2. Write a Python script to perform DNS queries: The script will perform NS (Name Server) queries on the subdomains of the GCP domain. If the NS record points to a domain that is not controlled by the owner of the GCP domain, it is a misconfiguration. Here is a simple script to perform this check:
...
def check_ns_delegation(domain):
    try:
        ...
        for rdata in answers:
            print('NS record for {}: {}'.format(domain, rdata.to_text()))
    except Exception as e:
        print('Error querying NS record for {}: {}'.format(domain, e))

# Replace 'subdomain.example.com' with the subdomain you want to check
check_ns_delegation('subdomain.example.com')
```

3. Run the script: You can run the script using the Python interpreter. The script will print the NS records for the specified subdomain. If the NS record points to a domain that is not controlled by the owner of the GCP domain, it is a misconfiguration.

4. Analyze the results: If the NS record for the subdomain points to a domain that is not controlled by the owner of the GCP domain, it is a misconfiguration. This can lead to subdomain takeover, where an attacker can claim the uncontrolled domain and serve malicious content.
