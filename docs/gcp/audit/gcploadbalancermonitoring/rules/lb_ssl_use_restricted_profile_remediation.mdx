...
2. Navigate to Load Balancer: From the main dashboard, click on the navigation menu (hamburger icon) located at the top left corner of the console. Scroll down and click on "Network Services" and then "Load balancing".

3. Select the Load Balancer: In the Load balancing page, you will see a list of all your load balancers. Click on the name of the load balancer you want to check.

4. Check SSL Policy: In the load balancer details page, scroll down to the "Frontend configuration" section. Here, you can see the SSL policy used by the load balancer. If the SSL policy is not set to "Restricted", then the load balancer is not using a restricted SSL profile.
...
2. Authenticate your CLI with your Google Cloud account. You can do this by running the following command and following the prompts:
...
4. For each load balancer, check the SSL policy used. You can do this by running the following command:

   ```
   gcloud compute ssl-policies describe [SSL_POLICY] --format="get(profile)"
   ```

   Replace `[SSL_POLICY]` with the name of the SSL policy used by the load balancer. This command will return the profile of the SSL policy. If the profile is not "RESTRICTED", then the load balancer is not using a restricted profile.

#### Using Python

1. AWS:
   - Install boto3, the AWS SDK for Python, and configure it with your credentials.
   - Use the `elbv2` client to describe your load balancers and their listeners.
   - For each listener, check the SSL policy. If it's not a restricted policy, flag it.
   - Python script:
     ```python
     import boto3
     ...
     for lb in response['LoadBalancers']:
         ...
         for listener in listeners['Listeners']:
             if 'SslPolicy' in listener and not listener['SslPolicy'].startswith('ELBSecurityPolicy-TLS-1-2-'):
                 print(f"Load balancer {lb['LoadBalancerName']} uses unrestricted SSL policy {listener['SslPolicy']}")
     ```

2. Azure:
   - Install the Azure SDK for Python and configure it with your credentials.
   - Use the `NetworkManagementClient` to get your load balancers.
   - For each load balancer, get its front end IP configurations, then get its associated inbound NAT rules.
   - For each inbound NAT rule, check the protocol. If it's not 'Https', flag it.
   - Python script:
     ```python
     ...
     for lb in network_client.load_balancers.list_all():
         for config in lb.frontend_ip_configurations:
             for rule in config.inbound_nat_rules:
                 if rule.protocol != 'Https':
                     print(f"Load balancer {lb.name} uses unrestricted protocol {rule.protocol}")
     ```

3. GCP:
   - Install the Google Cloud SDK for Python and configure it with your credentials.
   - Use the `compute` service to get your target HTTPS proxies.
   - For each proxy, get its SSL policy. If it's not a restricted policy, flag it.
   - Python script:
     ```python
     from google.cloud import compute_v1
     client = compute_v1.TargetHttpsProxiesClient()
     for proxy in client.list(project='your-project-id'):
         ssl_policy = client.get_ssl_policy(proxy=proxy.name, project='your-project-id')
         if ssl_policy.min_tls_version != 'TLS_1_2':
             print(f"Target HTTPS proxy {proxy.name} uses unrestricted SSL policy {ssl_policy.min_tls_version}")
     ```
