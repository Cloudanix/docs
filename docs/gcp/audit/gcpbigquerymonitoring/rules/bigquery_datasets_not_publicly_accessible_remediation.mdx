...
2. Navigate to the BigQuery service. You can do this by clicking on the navigation menu (hamburger icon) in the top left corner, then scroll down and click on "BigQuery" under the "Big Data" section.
3. Once you're in the BigQuery interface, you'll see a list of your datasets on the left side of the screen. Click on the name of the dataset that you want to check.
4. After selecting a dataset, click on the "SHARE DATASET" button at the top of the page. This will open a new window where you can see the dataset's access controls. If "All authenticated users" or "All users" are listed in the dataset permissions, this means the dataset is publicly or anonymously accessible.
...
3. For each dataset, you can check its access controls using the following command:

   ```
   bq show --format=prettyjson [PROJECT_ID]:[DATASET]
   ```
   Replace `[PROJECT_ID]` with your project ID and `[DATASET]` with the name of the dataset you want to check. This command will return a JSON object that includes the dataset's access controls.

4. In the returned JSON, look for the "access" field. This field contains an array of objects, each representing a different access control rule. If any of these objects have a "role" of "READER" and a "specialGroup" of "allAuthenticatedUsers" or "allUsers", then the dataset is accessible to all authenticated users or the public, respectively.
...
4. Analyze the output: The script will print out the IDs of all datasets that are either publicly accessible or accessible by any authenticated user. If no such datasets are found, the script will not output anything. This can be used to detect misconfigurations where BigQuery datasets are anonymously or publicly accessible.
