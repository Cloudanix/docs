...
3. Access BigQuery Dataset: In the BigQuery console, you will see a list of all your BigQuery datasets. Click on the dataset that contains the table you want to check.

4. Check Table Details: Click on the table you want to check. This will open the table details page. Here, under the "Details" tab, look for the "Options" section. If the "Ignore Unknown Values" option is not enabled (i.e., it is set to "No"), then the BigQuery table is not configured to ignore unknown values.
...
5. For each dataset, list all tables and check if the 'ignoreUnknownValues' property is set to true. Replace `your-dataset-id` with your actual dataset ID.
   ```
   gcloud bigquery tables describe your-dataset-id
   ```
   If the 'ignoreUnknownValues' property is not listed or set to false, then the BigQuery table does not have the 'Ignore Unknown Values' option enabled.
...
3. Fetch the list of datasets and tables: Now, you can fetch the list of datasets in your project and iterate over them to fetch the list of tables in each dataset.
...
4. Check the 'ignore_unknown_values' property: For each table, fetch the table details and check the 'ignore_unknown_values' property. If it's set to False, then the table does not have 'Ignore Unknown Values' enabled.
   ...
   for table in tables:
       ...
       if not table_obj.schema:
           print(f"Table {table.table_id} does not have a schema set.")
       elif not table_obj.ignore_unknown_values:
           print(f"Table {table.table_id} does not have 'Ignore Unknown Values' enabled.")
   ```

This script will print out the IDs of all tables that do not have 'Ignore Unknown Values' enabled. You can modify it to suit your needs, for example by writing the output to a file or sending an alert.
