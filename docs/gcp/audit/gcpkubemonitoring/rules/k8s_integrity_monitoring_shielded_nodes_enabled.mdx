...
3. Select the Cluster: From the list of available clusters, select the one you want to check for the integrity monitoring configuration. This will open the cluster's details page.

4. Check the Shielded GKE Nodes Configuration: In the cluster's details page, scroll down to the "Security" section. Look for the "Shielded GKE Nodes" setting. If the "Enable Integrity Monitoring" option is enabled, it means that Integrity Monitoring for Shielded GKE Nodes is enabled. If it's not, then it's a misconfiguration.

#### Using CLI

1. First, you need to install and configure the Google Cloud SDK (gcloud CLI) on your local machine. You can download it from the official Google Cloud website and follow the instructions for installation.
...
3. Now, you can list all the clusters in your project by running the following command:
   ...
   ```
   This will display a list of all the clusters in your project, along with their details.

4. To check if Integrity Monitoring for Shielded GKE Nodes is enabled, you need to describe each cluster and look for the "enableIntegrityMonitoring" field. You can do this by running the following command for each cluster:
   ```
   gcloud container clusters describe [CLUSTER_NAME] --format=json
   ```
   Replace [CLUSTER_NAME] with the name of your cluster. This command will output the configuration of the cluster in JSON format. Look for the "shieldedNodes" field and check if "enableIntegrityMonitoring" is set to true. If it is, then Integrity Monitoring for Shielded GKE Nodes is enabled.

#### Using Python

To check if Integrity Monitoring for Shielded GKE Nodes is enabled in Kubernetes using Python scripts, you can follow these steps:

1. **Import the Required Libraries**: You will need to import the Google Cloud Client Library for Python. If you haven't installed it yet, you can do so using pip:
...
2. **Initialize the Kubernetes Client**: Initialize the Kubernetes client in your Python script. You will need to authenticate using your Google Cloud credentials. 
...
3. **List all the Clusters**: Use the client to list all the clusters in your project and zone. 

```python
def list_clusters(client, project_id, zone):
    response = client.list_clusters(project_id, zone)
    return response
```

4. **Check the Shielded Nodes Status**: For each cluster, check if the shielded GKE nodes are enabled. 

```python
def check_shielded_nodes_status(client, project_id, zone):
    clusters = list_clusters(client, project_id, zone)
    for cluster in clusters:
        if not cluster.shielded_nodes.enabled:
            print(f"Shielded Nodes are not enabled for cluster {cluster.name}")
```

This script will print out the names of all clusters where Shielded Nodes are not enabled. You can modify it to suit your needs, for example by raising an exception or sending an alert if any misconfigurations are found.
