...
3. In the Kubernetes Engine page, you will see a list of all your Kubernetes clusters. Look for the column labeled "Release Channel". If a cluster is in the "Rapid" or "Regular" channel, it is considered an Alpha cluster.

4. Check the details of each cluster by clicking on the cluster name. In the details page, look for the "Master version" field. If the version is prefixed with "1.7" or lower, it is considered an Alpha cluster.
...
2. List all the clusters: Use the following command to list all the Kubernetes clusters in your GCP account. This command will return a list of all clusters along with their details.

    ```
    gcloud container clusters list
    ```

3. Check the version of each cluster: For each cluster, check the version of the Kubernetes master and the version of the nodes. You can do this using the following command:

    ```
    gcloud container clusters describe [CLUSTER_NAME] --zone [ZONE_NAME]
    ```

    Replace `[CLUSTER_NAME]` with the name of your cluster and `[ZONE_NAME]` with the zone of your cluster. This command will return a lot of information about your cluster. Look for the `currentMasterVersion` and `currentNodeVersion` fields. If these fields have a value that includes `alpha`, then the cluster is using an alpha version.

4. Automate the process: If you have many clusters, it might be tedious to check each one manually. You can automate this process using a script. Here is a simple Python script that uses the `subprocess` module to run the GCP CLI commands and check the versions of all clusters:

    ```python
    import subprocess

    # Get the list of all clusters
    clusters = subprocess.check_output(['gcloud', 'container', 'clusters', 'list']).decode('utf-8').split('\n')

    for cluster in clusters:
        # Get the details of the cluster
        details = subprocess.check_output(['gcloud', 'container', 'clusters', 'describe', cluster]).decode('utf-8')

        # Check if the cluster is using an alpha version
        if 'alpha' in details:
            print(f'Cluster {cluster} is using an alpha version.')
    ```

    This script will print the names of all clusters that are using an alpha version.
