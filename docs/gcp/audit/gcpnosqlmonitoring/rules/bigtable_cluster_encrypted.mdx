---
slug: bigtable_cluster_encrypted
title: Bigtable Cluster Should Be Encrypted
sidebar_label: Bigtable Cluster Should Be Encrypted
---

### More Info:

Ensure that Bigtable clusters are encrypted

### Risk Level

High

### Address

Security

### Compliance Standards

SOC2, NIST, GDPR, ISO27001, HIPAA, HITRUST, NISTCSF, PCIDSS



<Tabs><Tab title='Cause'>
### Check Cause

#### Using Console

1. Log in to the Google Cloud Platform Console: Open your preferred web browser, navigate to the Google Cloud Platform web console and sign in with your Google account that has the necessary permissions to access GCP resources.

2. Navigate to Bigtable Instances: From the GCP Console Dashboard, select "Navigation Menu" in the top left corner. Scroll down and click on "Bigtable" under the "STORAGE" section. This will take you to the Bigtable instances page.

3. Select the Bigtable Instance: On the Bigtable instances page, you will see a list of all your Bigtable instances. Click on the name of the instance for which you want to check the encryption status.

4. Check Encryption Status: In the instance details page, look for the "Encryption" section. If the instance is encrypted, it will show "Google-managed key" or "Customer-managed key" depending on the type of encryption used. If the instance is not encrypted, it will show "None".

#### Using CLI

1. Install and configure the Google Cloud SDK CLI on your local machine. You can download it from the official Google Cloud website and follow the instructions to set it up.

2. Authenticate your GCP account using the following command. This will open a new window in your web browser asking you to log in with your Google account and allow the Google Cloud SDK to access it.
   ```
   gcloud auth login
   ```

3. Set your GCP project ID to the project you want to check the Bigtable encryption for. Replace 'your-project-id' with your actual project ID.
   ```
   gcloud config set project your-project-id
   ```

4. Run the following command to list all Bigtable instances in your project and their encryption info. This will return a list of all Bigtable instances along with their encryption status.
   ```
   gcloud bigtable instances list --format="table[box](name,displayName,create_time.date('%Y-%m-%dT%H:%M:%S%Oz'),state,type,encryptionInfo)"
   ```
   If the encryptionInfo field is empty or shows 'Google managed key', it means that the Bigtable cluster is not encrypted with a customer-managed key.

#### Using Python

To check if a Bigtable Cluster is encrypted in NoSQL using Python scripts, you can follow these steps:

1. **Setup Google Cloud SDK and Python Environment:**
   Before you start, make sure you have the Google Cloud SDK installed and initialized and also have Python environment set up with 'google-cloud-bigtable' client library installed.

2. **Import Required Libraries and Set Up Client:**
   Import the required libraries and set up the Bigtable client. Here is a sample script:

   ```python
   from google.cloud import bigtable
   client = bigtable.Client()
   ```

3. **List All Bigtable Instances and Clusters:**
   Use the `list_instances()` method to get all the Bigtable instances. For each instance, use the `list_clusters()` method to get all the clusters. Here is a sample script:

   ```python
   instances = client.list_instances()
   for instance in instances:
       clusters = instance.list_clusters()
       for cluster in clusters:
           print(cluster)
   ```

4. **Check Encryption Status:**
   For each cluster, check the `encryption_config` attribute. If it's `None` or not set to the desired encryption type, then the cluster is not encrypted. Here is a sample script:

   ```python
   for instance in instances:
       clusters = instance.list_clusters()
       for cluster in clusters:
           if cluster.encryption_config is None:
               print(f"Cluster {cluster.cluster_id} is not encrypted.")
           else:
               print(f"Cluster {cluster.cluster_id} is encrypted with {cluster.encryption_config.encryption_type}.")
   ```

Please note that the above scripts are just samples and may need to be adjusted based on your specific environment and requirements. Also, the scripts assume that you have the necessary permissions to list and view Bigtable instances and clusters.

</Tab>


<Tab title='Remediation'>
### Remediation

#### Using Console

To remediate the misconfiguration "Bigtable Cluster should be encrypted" in GCP, you can follow the below steps using the GCP console:

1. Open the GCP console and navigate to the Bigtable instance that needs to be encrypted.

2. Click on the "Edit" button on the top of the page.

3. Scroll down to the "Encryption" section and click on the "Edit" button next to it.

4. Select the "Customer-managed key" option and choose the key that you want to use for encryption.

5. Click on the "Save" button to save the changes.

6. Once the changes are saved, the Bigtable cluster will be encrypted using the selected customer-managed key.

7. Verify the encryption status of the Bigtable cluster by checking the "Encryption" section on the Bigtable instance page. It should show that the cluster is encrypted using the selected customer-managed key.

By following these steps, you can remediate the misconfiguration "Bigtable Cluster should be encrypted" in GCP using the GCP console.

#### Using CLI

To remediate the misconfiguration of a non-encrypted Bigtable cluster in GCP, you can follow these steps using GCP CLI:

1. Open the Cloud Shell in your GCP console.

2. Check the current status of the Bigtable cluster by running the following command:

```
gcloud beta bigtable clusters describe [CLUSTER_ID] --project=[PROJECT_ID] --instance=[INSTANCE_ID]
```

3. If the output shows that the cluster is not encrypted, then run the following command to enable encryption:

```
gcloud beta bigtable clusters update [CLUSTER_ID] --project=[PROJECT_ID] --instance=[INSTANCE_ID] --encryption-at-rest-state=ENABLED
```

4. Wait for the update to complete. This may take a few minutes.

5. Verify that the encryption is enabled by running the following command:

```
gcloud beta bigtable clusters describe [CLUSTER_ID] --project=[PROJECT_ID] --instance=[INSTANCE_ID]
```

6. Check the output to confirm that the encryption-at-rest-state is set to ENABLED.

7. Once you have confirmed that the encryption is enabled, you have successfully remediated the non-encrypted Bigtable cluster misconfiguration.

#### Using Python

To remediate the misconfiguration "Bigtable Cluster Should Be Encrypted" in GCP using python, you can follow the below steps:

1. Open the GCP console and navigate to the Bigtable Clusters page.

2. Select the Bigtable cluster that needs to be encrypted.

3. Click on the "Edit" button to edit the cluster configuration.

4. In the "Security" section, check if the "Encryption" option is enabled. If not, enable it.

5. Choose the encryption type that you want to use. You can choose between Google-managed encryption keys or customer-managed encryption keys.

6. If you choose customer-managed encryption keys, provide the key name and key version.

7. Save the changes to update the Bigtable cluster configuration.

8. To automate this process using python, you can use the GCP SDK libraries. Here is an example code snippet to enable encryption for a Bigtable cluster using python:

```
from google.cloud import bigtable

client = bigtable.Client(project='your-project-id')
instance = client.instance('your-instance-id')
cluster = instance.cluster('your-cluster-id')

# Enable encryption
cluster.encryption_at_rest_type = 'GOOGLE_DEFAULT_ENCRYPTION'
cluster.update()
```

In the above code snippet, replace "your-project-id", "your-instance-id" and "your-cluster-id" with the actual values for your Bigtable cluster. Also, you can choose the encryption type as per your requirement.


### Additional Reading:

- [https://cloud.google.com/bigtable/docs/cmek](https://cloud.google.com/bigtable/docs/cmek) 


</Tab>
</Tabs>