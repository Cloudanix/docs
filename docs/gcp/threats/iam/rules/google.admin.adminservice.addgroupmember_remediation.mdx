...
- The google.admin.AdminService.addGroupMember event in GCP for IAM refers to an event where a user is added as a member to a group in the Google Cloud Identity and Access Management (GCPIAM) service.
