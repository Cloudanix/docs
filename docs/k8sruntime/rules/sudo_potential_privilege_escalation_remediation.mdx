...
1. Identify the user or process involved in the event by checking the relevant logs or audit trails.
   - Use the following command to view the logs of the Kubernetes cluster:
     ```
     kubectl logs <falco-pod-name> -n <falco-namespace>
     ```
...
1. Identify the affected pod(s) by checking the `output` field in the Falco event. This field contains the command executed by the user. Use the `kubectl` command to get the pod details:
