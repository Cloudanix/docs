...
3. Check SQL Server settings: After you've navigated to the SQL servers page, you'll see a list of all your SQL servers. Click on the name of the server you want to check. This will take you to the server's overview page. From here, click on "Security" in the left-hand menu, then click on "Advanced Threat Protection".

4. Check Threat Detection Alerts: In the Advanced Threat Protection settings, look for the "Threat Detection Alerts" section. If this feature is disabled, it means that you won't receive any alerts about potential threats to your SQL server.
...
3. Set your Azure Subscription: If you have multiple subscriptions, set the subscription you want to use. Replace 'my-subscription-name' with your subscription name.
   ...
   ```
4. Check Threat Detection Alerts: Now, you can check the Threat Detection Alerts for SQL Servers. Replace 'resource-group-name' and 'server-name' with your resource group and server name respectively.
   ```
   az sql server threat-policy show --resource-group 'resource-group-name' --server 'server-name'
   ```
   In the output, look for the "state" field. If it is 'Disabled', then Threat Detection Alerts are disabled for that SQL Server.
...
3. Iterate over SQL servers and databases: Now, you can iterate over all SQL servers and databases in your subscription and check if Threat Detection is enabled.

   ```python
   for server in client.servers.list():
       for database in client.databases.list_by_server(server.resource_group, server.name):
           threat_policy = client.database_threat_detection_policies.get(
               server.resource_group, server.name, database.name)
           if threat_policy.state != 'Enabled':
               print(f'Threat Detection is disabled for SQL Server {server.name} and Database {database.name}')
   ```

4. Run the script: Finally, you can run the script. If Threat Detection is disabled for any SQL Server and Database, it will print a message with the server and database name. If no message is printed, it means Threat Detection is enabled for all SQL Servers and Databases in your subscription.
