...
1. Login to Azure Portal: Sign in to the Azure portal using your Azure account credentials.

2. Navigate to PostgreSQL Servers: From the left-hand menu, select "All services". In the "All services" box, search for "PostgreSQL servers". Select it to open the PostgreSQL servers dashboard.

3. Check Firewall Rules: Select the PostgreSQL server for which you want to check the network access. In the server dashboard, select "Firewall rules" under the "Security" section. Here, you can see all the firewall rules that are currently in place for your PostgreSQL server.

4. Check for Unrestricted Access: In the firewall rules, look for any rule that has the "Start IP" as "0.0.0.0" and the "End IP" as "255.255.255.255". This rule allows access to your PostgreSQL server from any IP address, indicating unrestricted database access.
...
2. List all PostgreSQL servers: Use the following command to list all the PostgreSQL servers in your Azure account. This command will return a list of all PostgreSQL servers along with their details.

    ```
    az postgres server list --output table
    ```
    This command will display the list of PostgreSQL servers in a table format. Look for the 'name' and 'resourceGroup' of the PostgreSQL server you want to check.

3. Check the network rules: Now, use the following command to list all the network rules for the PostgreSQL server. Replace `<server_name>` and `<resource_group>` with the name and resource group of your PostgreSQL server.

    ```
    az postgres server vnet-rule list --name <server_name> --resource-group <resource_group> --output table
    ```
    This command will display a list of all network rules for the PostgreSQL server in a table format. Look for the 'startIpAddress' and 'endIpAddress' in the output.

4. Detect unrestricted access: If the 'startIpAddress' is '0.0.0.0' and the 'endIpAddress' is '255.255.255.255', then the PostgreSQL server has unrestricted network access. This is a misconfiguration as it allows any IP address to access the PostgreSQL server.

#### Using Python

1. AWS:
   - Use the boto3 library in Python to interact with AWS services. 
   - To check for unrestricted PostgreSQL database access, you would need to list all the security groups and check their ingress rules.
    ...
    for security_group in ec2.security_groups.all():
        for permission in security_group.ip_permissions:
            for ip_range in permission['IpRanges']:
                if ip_range['CidrIp'] == '0.0.0.0/0' and permission['FromPort'] == 5432:
                    print(f"Unrestricted PostgreSQL access detected in security group: {security_group.id}")

check_security_groups()
```

2. Azure:
   - Use the azure-mgmt-network library in Python to interact with Azure services.
   - To check for unrestricted PostgreSQL database access, you would need to list all the network security groups and check their security rules.
   - Here is a sample script:
    ...
    for security_group in network_client.network_security_groups.list_all():
        for rule in security_group.security_rules:
            if rule.destination_port_range == '5432' and rule.source_address_prefix == '*':
                print(f"Unrestricted PostgreSQL access detected in security group: {security_group.name}")

check_security_groups()
```

3. GCP:
   - Use the google-cloud-compute library in Python to interact with GCP services.
   - To check for unrestricted PostgreSQL database access, you would need to list all the firewall rules and check their allowed configurations.
   - Here is a sample script:
...
def check_firewall_rules():
    ...
    for firewall in client.list(project='your-project-id'):
        for allowed in firewall.allowed:
            if allowed.IPProtocol == 'tcp' and '5432' in allowed.ports and '0.0.0.0/0' in firewall.source_ranges:
                print(f"Unrestricted PostgreSQL access detected in firewall rule: {firewall.name}")
