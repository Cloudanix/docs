---
slug: azure_audit_networksecurity_security_group_unrestricted_netbios_access
title: Check for Unrestricted Netbios Access
sidebar_label: Check for Unrestricted Netbios Access
---

### More Info:

Ensure that no network security groups allow unrestricted inbound access on TCP port 139 and UDP ports 137 and 138 (NetBIOS).

### Risk Level

High

### Address

Security

### Compliance Standards

HIPAA, NIST, HITRUST, GDPR, SOC2, NISTCSF, PCIDSS, FedRAMP


<Tabs><Tab title='Cause'>
### Check Cause

#### Using Console

1. Login to Azure Portal: Open your web browser and navigate to the Azure portal. Sign in with your Azure account credentials.

2. Navigate to Network Watcher: On the left-hand side menu, click on "All services". In the search box, type "Network Watcher" and select it from the dropdown list.

3. Check Security Group Rules: In the Network Watcher dashboard, click on "Effective security rules" under the "Security Group View" section. This will display all the security rules applied to your network interfaces.

4. Inspect Rules for NetBIOS Access: Look for any rules that allow inbound or outbound traffic on ports 137, 138, and 139, which are used by NetBIOS. If such rules are found and they are not restricted to specific IP addresses, it indicates unrestricted NetBIOS access, which is a misconfiguration.

#### Using CLI

1. Install Azure CLI: Before you can run any Azure CLI commands, you need to install Azure CLI on your local machine. You can download it from the official Microsoft Azure website. Once installed, open your command prompt and type `az login` to log in to your Azure account.

2. List Network Security Groups: The first step to detect unrestricted NetBIOS access is to list all the network security groups in your Azure account. You can do this by running the following command: `az network nsg list --query "[].{Name:name, ResourceGroup:resourceGroup}" -o table`. This command will display a table with the names of all your network security groups and their respective resource groups.

3. Check Security Rules: For each network security group, you need to check the security rules to see if any of them allow unrestricted NetBIOS access. You can do this by running the following command for each network security group: `az network nsg rule list --nsg-name <NSG Name> --resource-group <Resource Group Name> --query "[?access=='Allow' && (protocol=='All' || protocol=='Tcp') && (destinationPortRange=='137-139' || destinationPortRange=='445')]" -o table`. Replace `<NSG Name>` and `<Resource Group Name>` with the name of the network security group and the name of the resource group respectively. This command will display a table with the security rules that allow unrestricted NetBIOS access.

4. Analyze the Output: If the output from the previous step is not empty, it means that unrestricted NetBIOS access is allowed. You should see a list of security rules that allow traffic on ports 137-139 and 445, which are the ports used by NetBIOS. If the output is empty, it means that unrestricted NetBIOS access is not allowed.

#### Using Python

1. AWS:
   - Use the AWS SDK for Python (Boto3) to interact with AWS services. To check for unrestricted NetBIOS access, you can use the EC2 client to describe security groups and check their ingress rules.
   - Here is a sample script:
   ```python
   import boto3

   ec2 = boto3.client('ec2')

   response = ec2.describe_security_groups()

   for group in response['SecurityGroups']:
       for permission in group['IpPermissions']:
           if 'FromPort' in permission and permission['FromPort'] == 139:
               for range in permission['IpRanges']:
                   if range['CidrIp'] == '0.0.0.0/0':
                       print(f"Unrestricted NetBIOS access detected in security group: {group['GroupId']}")
   ```
2. Azure:
   - Use the Azure SDK for Python to interact with Azure services. To check for unrestricted NetBIOS access, you can use the NetworkManagementClient to list all network security groups and check their security rules.
   - Here is a sample script:
   ```python
   from azure.mgmt.network import NetworkManagementClient
   from azure.identity import DefaultAzureCredential

   credential = DefaultAzureCredential()
   subscription_id = 'your-subscription-id'
   network_client = NetworkManagementClient(credential, subscription_id)

   for nsg in network_client.network_security_groups.list_all():
       for rule in nsg.security_rules:
           if rule.destination_port_range == '139' and rule.source_address_prefix == '*':
               print(f"Unrestricted NetBIOS access detected in network security group: {nsg.id}")
   ```
3. GCP:
   - Use the Google Cloud Client Library for Python to interact with GCP services. To check for unrestricted NetBIOS access, you can use the Compute Engine client to list all firewalls and check their allowed configurations.
   - Here is a sample script:
   ```python
   from google.cloud import compute_v1

   client = compute_v1.FirewallsClient()

   for firewall in client.list(project='your-project-id'):
       for allowed in firewall.allowed:
           if allowed.IPProtocol == 'tcp' and '139' in allowed.ports:
               for source_range in firewall.source_ranges:
                   if source_range == '0.0.0.0/0':
                       print(f"Unrestricted NetBIOS access detected in firewall: {firewall.name}")
   ```

</Tab>


<Tab title='Remediation'>
### Remediation

#### Using Console

Here are the step-by-step instructions to remediate the Unrestricted Netbios Access misconfiguration in Azure:

1. Log in to the Azure portal.
2. Go to the "Virtual networks" page.
3. Select the virtual network that you want to remediate.
4. Click on the "Subnets" option in the left-hand menu.
5. Select the subnet that you want to remediate.
6. Click on the "Network security group" option in the left-hand menu.
7. Click on the "Edit" button to edit the network security group associated with the subnet.
8. Click on the "Inbound security rules" option in the left-hand menu.
9. Find the rule that allows unrestricted NetBIOS access.
10. Click on the rule to select it.
11. Click on the "Delete" button to delete the rule.
12. Click on the "Save" button to save the changes.

By following these steps, you have successfully remediated the Unrestricted Netbios Access misconfiguration in Azure.

#### Using CLI

To remediate Unrestricted NetBIOS Access in Azure using Azure CLI, follow the below steps:

Step 1: Open Azure CLI and login to your Azure account.

Step 2: Run the below command to list all the network security groups in your subscription:

```
az network nsg list
```

Step 3: Identify the NSG that is associated with your virtual machine or subnet that has Unrestricted NetBIOS Access.

Step 4: Run the below command to get the details of the NSG:

```
az network nsg show --name <nsg-name> --resource-group <resource-group-name>
```

Step 5: Identify the security rule that allows Unrestricted NetBIOS Access.

Step 6: Run the below command to delete the security rule:

```
az network nsg rule delete --name <rule-name> --nsg-name <nsg-name> --resource-group <resource-group-name>
```

Note: Replace `<rule-name>`, `<nsg-name>` and `<resource-group-name>` with the actual values.

Step 7: Verify that the security rule is deleted by running the below command:

```
az network nsg show --name <nsg-name> --resource-group <resource-group-name>
```

Step 8: Repeat the above steps for all the NSGs that have Unrestricted NetBIOS Access.

By following the above steps, you can remediate Unrestricted NetBIOS Access in Azure using Azure CLI.

#### Using Python

To remediate Unrestricted Netbios Access in Azure using Python, you can follow the below steps:

Step 1: Install the Azure SDK for Python using pip.

```
pip install azure
```

Step 2: Authenticate with Azure using the below code.

```python
from azure.common.credentials import ServicePrincipalCredentials
from azure.mgmt.network import NetworkManagementClient

subscription_id = 'your_subscription_id'
credentials = ServicePrincipalCredentials(
    client_id='your_client_id',
    secret='your_secret',
    tenant='your_tenant_id'
)

network_client = NetworkManagementClient(
    credentials,
    subscription_id
)
```

Step 3: Get the Network Security Group (NSG) that has unrestricted Netbios access using the below code.

```python
nsg_name = 'your_nsg_name'
resource_group_name = 'your_resource_group_name'

nsg = network_client.network_security_groups.get(
    resource_group_name,
    nsg_name
)
```

Step 4: Remove the rule that allows unrestricted Netbios access using the below code.

```python
rule_name = 'your_rule_name'

for rule in nsg.security_rules:
    if rule.name == rule_name:
        nsg.security_rules.remove(rule)
        break

poller = network_client.network_security_groups.create_or_update(
    resource_group_name,
    nsg_name,
    nsg
)
poller.wait()
```

This code will remove the rule that allows unrestricted Netbios access from the NSG. You can replace the `your_nsg_name`, `your_resource_group_name`, and `your_rule_name` with your own values.




</Tab>
</Tabs>