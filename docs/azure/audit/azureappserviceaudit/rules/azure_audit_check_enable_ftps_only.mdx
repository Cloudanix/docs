...
2. Navigate to the App Services and select the App Service you want to check.

3. In the left-hand menu, under the 'Settings' section, click on 'Configuration'.

4. In the 'Configuration' page, select the 'General settings' tab. Look for the 'FTPS state' setting. If it is set to 'FTPS Only', then FTPS-Only Access is enabled. If it is set to 'All Allowed' or 'Disabled', then FTPS-Only Access is not enabled.
...
2. Login to Azure: Use the following command to login to your Azure account. You will be prompted to enter your login credentials.
   ...
   az login
   ```
3. Select the subscription: If you have multiple subscriptions, select the subscription where the App Service is located using the following command. Replace 'your-subscription-id' with your actual subscription ID.
   ```
   az account set --subscription 'your-subscription-id'
   ```
4. Check FTPS state: Use the following command to check the FTPS state of your App Service. Replace 'your-resource-group' with the name of your resource group and 'your-app-service' with the name of your App Service.
   ```
   az webapp config show --resource-group 'your-resource-group' --name 'your-app-service' --query 'ftpsState'
   ```
   If the output is 'FtpsOnly', then FTPS-Only Access is enabled for your App Service. If the output is 'AllAllowed' or 'Disabled', then FTPS-Only Access is not enabled.
...
2. Import necessary modules: Import the necessary modules in your Python script. You will need the `azure.mgmt.web` module to interact with App Services and `azure.common.credentials` to authenticate your script.
   ```python
   from azure.common.credentials import ServicePrincipalCredentials
   from azure.mgmt.web import WebSiteManagementClient
   ```

3. Authenticate your script: You need to authenticate your script to interact with your Azure resources. You can do this using a service principal. Replace `tenant`, `client_id`, and `secret` with your Azure AD tenant ID, service principal client ID, and client secret respectively.
   ```python
...
4. Check FTPS state: Now you can check the FTPS state of your App Service. Replace `subscription_id`, `resource_group`, and `name` with your Azure subscription ID, resource group name, and App Service name respectively.
   ```python
   client = WebSiteManagementClient(credentials, 'subscription_id')
   site_config = client.web_apps.get_configuration('resource_group', 'name')
   ftps_state = site_config.ftps_state
   if ftps_state != 'FtpsOnly':
       print('FTPS-Only Access is not enabled for this App Service')
   else:
       print('FTPS-Only Access is enabled for this App Service')
   ```
   This script will print whether FTPS-Only Access is enabled for the specified App Service. If it is not, you have detected a misconfiguration.
